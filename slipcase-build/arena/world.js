...
function emit(S) {
  ...
  for (const [noun, g] of Object.entries(S.library || {})) if ((S.instances || []).some(i => i.noun === noun)) subs.push([`0 FILE ${noun}.ldr`, `0 ${noun}`, `0 Name: ${noun}.ldr`].concat(g.map(q => `1 ${q.color} ${q.pos[0]} ${q.pos[1]} ${q.pos[2]} ${q.mat.join(' ')} ${q.part}.dat`)));
