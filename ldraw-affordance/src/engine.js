...
export function compatibility(pa,pb,rules){
  ...
  return {
    joint:rule.joint,
    normalRelation:rule.normalRelation||'opposed',
    motion:rule.motion||'seat',
    approach:rule.approach||12,
    tax:rule.baseTax+(conf[pa.confidence]||0)+(conf[pb.confidence]||0)
  };
}
...
export function snapChild(parentInst,parentPort,childPart,childPort,connection={normalRelation:'opposed'}){
  ...
  for(const r of ORIENTATIONS){
    ...
    if(dot(cn,desired)<0.999) continue;
...
export function inspectSeam(parentInst,parentPort,childInst,childPort,connection,rules={}){
  ...
  const confidence=[parentPort.confidence,childPort.confidence].includes('approx')?'CALIBRATE':'CLICKED';
  return {posError,normalDot,ok:posError<=positionTolerance&&normalDot>=normalTolerance,status:posError<=positionTolerance&&normalDot>=normalTolerance?confidence:'OPEN'};
}
export function bestConnection(parentInst,parentPart,parentPortId,childPart,rules){
  ...
  for(const chosen of candidates){
    const snapped=snapChild(parentInst,pp,childPart,chosen.cp,chosen.c);
    if(!snapped) continue;
    const childInst={partId:childPart.id,t:snapped.t,r:snapped.r};
    const seam=inspectSeam(parentInst,pp,childInst,chosen.cp,chosen.c,rules);
    if(seam.ok) return {...snapped,parentPortId,childPortId:chosen.cp.id,...chosen.c,seam};
  }
  return null;
}
export function findCoincidentConnections(assembly,childInst,index,rules){
  const childPart=index.get(childInst.partId),out=[];
  if(!childPart) return out;
  const childUsed=new Set(childInst.usedPorts||[]);
  for(const other of assembly){
    if(other===childInst) continue;
    const otherPart=index.get(other.partId); if(!otherPart) continue;
    const otherUsed=new Set(other.usedPorts||[]);
    for(const pp of otherPart.ports){
      if(otherUsed.has(pp.id)) continue;
      for(const cp of childPart.ports){
        if(childUsed.has(cp.id)) continue;
        const c=compatibility(pp,cp,rules); if(!c) continue;
        const seam=inspectSeam(other,pp,childInst,cp,c,rules);
        if(seam.ok) out.push({other,pp,cp,connection:c,seam});
      }
    }
  }
  return out;
