...
import {makePlan,simulate,signalLoad,pressureMissing,thrustMissing,DESIGN} from '../beaver-spaceship/model-runtime.js';
...
assert.equal(sim.thrustMissing.length,0,`thrust stations remain: ${sim.thrustMissing.join(', ')}`);
...
assert.equal(engineSim.remaining.length,0,'remaining plan should still assemble after engine ablation');
assert.deepEqual(thrustMissing(engineSim.state),['T|port|28'],'removing an engine must keep exactly that thrust station unresolved');
...
console.log(`PASS · pressure gaps ${sim.pressureMissing.length} · thrust gaps ${sim.thrustMissing.length} · ${files.length} real LDraw files`);
