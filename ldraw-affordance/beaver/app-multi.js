...
const params=new URLSearchParams(location.search),requested=params.get('build');
let buildIndex=Math.max(0,BUILDS.findIndex(b=>b.id===requested)),vocabMode='full',solver=null,running=false,audioCtx=null,lastPoint=null,lastOk=true,suiteResults=new Map();
...
function serious(){return BUILDS[buildIndex]?.tier==='serious'}
...
function cueMarkers(){
  const g=new THREE.Group(),h=solver.hear(),rows=h.unresolved.slice(0,serious()?24:80);markers=[];
  for(const [i,s] of rows.entries()){
    const active=i===0,p=s.feature.prerequisite.p,n=s.feature.prerequisite.n,color=active?0xff3322:0xffa319,m=new THREE.Mesh(new THREE.SphereGeometry(active?3.6:1.8,10,7),new THREE.MeshBasicMaterial({color,depthTest:false}));
    m.position.set(...p);m.userData.p=i*.5;m.renderOrder=20;g.add(m);markers.push(m);g.add(new THREE.ArrowHelper(new THREE.Vector3(...n).normalize(),new THREE.Vector3(...p),active?16:9,color,3,2));
  }
  if(lastPoint){const m=new THREE.Mesh(new THREE.SphereGeometry(2.4,10,7),new THREE.MeshBasicMaterial({color:lastOk?0x00c970:0xff0033,depthTest:false}));m.position.set(...lastPoint);g.add(m)}return g
}
...
async function selectBuild(i){
  ...
  buildIndex=(i+BUILDS.length)%BUILDS.length;makeSolver();lastPoint=null;history.replaceState(null,'',`${location.pathname}?build=${encodeURIComponent(BUILDS[buildIndex].id)}`);setPhase('HEAR','read the strongest local releaser',BUILDS[buildIndex].objective==='bonded'?'bonded objective · maximize signal absorption through verified contacts':'same solver · different world');renderUI();await renderReal(true)
}
...
async function run(){
  ...
  try{
    const b=BUILDS[buildIndex],max=b.maxMoves||160,stride=Math.max(1,b.visualStride||1);let moves=0,guard=0;
...
async function runSuite(){
  ...
  try{
    for(let i=0;i<BUILDS.length;i++){
      buildIndex=i;makeSolver();let moves=0,guard=0,max=BUILDS[i].maxMoves||400;
      while(moves<max&&guard++<max*8){const r=solver.step();if(r.status==='retry')continue;latchStudMilestones();if(r.status!=='acted')break;moves++}
...
function reset(){if(running){renderUI();return}makeSolver();lastPoint=null;setPhase('HEAR','read the strongest local releaser',BUILDS[buildIndex].objective==='bonded'?'bonded objective · maximize signal absorption through verified contacts':'nothing placed without a physical test');renderUI();renderReal(true)}
async function singleStep(){if(running)return;audio();running=true;toggle(true);try{await act(true);scoreCurrent();renderUI()}catch(error){recoverUI(error,'HEAR · TRY · TEST')}finally{running=false;toggle(false)}}

$('#buildSelect').innerHTML=BUILDS.map((b,i)=>`<option value="${i}">${String(i+1).padStart(2,'0')} · ${b.tier==='serious'?'★ ':''}${b.name}</option>`).join('');
