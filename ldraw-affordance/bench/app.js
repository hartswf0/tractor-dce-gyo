...
import {loadIndex,compatibility,scoreSuite,ablation,varietyScore,bestConnection,snapChild,inspectSeam,findCoincidentConnections,transformVector,transformPoint,toLDraw,seamTax,seamLeakSeries,ID} from '../src/engine.js';
...
function leakColor(v){const t=Math.max(0,Math.min(1,v/.55));return new THREE.Color().setHSL((1-t)*.33,.78,.48)}
function seamMarkers(){
  const g=new THREE.Group(),series=seamLeakSeries(assembly,rules.repeatFloor??.15),byUid=new Map(assembly.map(x=>[x.uid,x]));
  for(const row of series){
    if(!row.parent||!row.via)continue;
    const parent=byUid.get(row.parent),pp=parent&&instancePart(parent)?.ports.find(p=>p.id===row.via.split(' ↔ ')[0]);if(!parent||!pp)continue;
    const p=transformPoint(parent,pp.p),r=1.4+Math.min(5,row.effectiveLeak*9),mesh=new THREE.Mesh(new THREE.SphereGeometry(r,14,10),new THREE.MeshBasicMaterial({color:leakColor(row.effectiveLeak),transparent:true,opacity:.86,depthTest:false}));
    mesh.position.set(...p);mesh.renderOrder=20;g.add(mesh);
  }
  return g;
}
try{const probe=await fetch('../../ldraw/LDConfig.ldr',{cache:'no-store'});if(!probe.ok)throw new Error(`LDConfig HTTP ${probe.status}`);await loader.preloadMaterials('../../ldraw/LDConfig.ldr');ldrawReady=true;$('#geomStatus').textContent='REAL LDRAW READY';$('#viewnote').textContent='Attention leaks at seams. Small/cool = solved. Large/warm = still demanding judgment.'}catch(err){console.error('[LDRAW INIT]',err);$('#geomStatus').textContent='LDRAW INIT ERROR';$('#viewnote').textContent=String(err?.message||err)}
function renderReal(){const serial=++renderSerial;if(!assembly.length){clearModel();$('#geomStatus').textContent=ldrawReady?'REAL LDRAW READY':'LDRAW NOT READY';return Promise.resolve()}if(!ldrawReady)return Promise.resolve();const mpd=toLDraw(assembly,index,'AFFORDANCE-BUILD');$('#geomStatus').textContent='PARSING REAL .DAT…';return new Promise(resolve=>{try{loader.parse(mpd,group=>{if(serial!==renderSerial){dispose(group);resolve();return}clearModel();modelWrapper=new THREE.Group();modelWrapper.rotation.x=Math.PI;modelWrapper.add(group);modelWrapper.add(seamMarkers());scene.add(modelWrapper);fit(modelWrapper);const s=geomStats(group);$('#geomStatus').textContent=`REAL LDRAW · ${assembly.length} PARTS · ${s.meshes} MESHES`;resolve()},err=>{console.error('[LDRAW PARSE]',err);$('#geomStatus').textContent='LDRAW BUILD ERROR';$('#viewnote').textContent=String(err?.message||err);resolve()})}catch(err){console.error('[LDRAW PARSE]',err);$('#geomStatus').textContent='LDRAW BUILD ERROR';resolve()}})}
...
async function attach(parent,portId,partId,color=16,label='',childPortId=null){
  ...
  const cp=part.ports.find(p=>p.id===snap.childPortId);usePort(parent,pp.id);
  const finalT=[...snap.t],out=transformVector(parent,pp.n),previewT=finalT.map((v,i)=>v+out[i]*(snap.approach||12));
  const leakKey=`${snap.joint}:${pp.confidence||'semantic'}:${cp?.confidence||'semantic'}`;
  const child={uid:crypto.randomUUID(),partId,t:previewT,r:snap.r,usedPorts:[snap.childPortId],seamTax:0,parent:parent.uid,joint:snap.joint,leakKey,via:`${pp.id} ↔ ${snap.childPortId}`,color,label,seamState:'APPROACH',seamCount:0,motion:snap.motion};
  assembly.push(child);selectedInstance=child;selectedPort=null;$('#trace').textContent=`${label||part.name} · APPROACH · seam not solved yet`;refresh();
  await delay(220);
  ...
  const row=seamLeakSeries(assembly,rules.repeatFloor??.15).at(-1);
  $('#trace').textContent=`${label||part.name} · ${child.seamState} · raw ${child.seamTax.toFixed(2)} → situated leak ${row.effectiveLeak.toFixed(3)}${row.repeatIndex>1?` · repetition #${row.repeatIndex}`:''}`;
...
function renderAssembly(){const el=$('#assemblyList');el.innerHTML='',series=seamLeakSeries(assembly,rules.repeatFloor??.15),leaks=new Map(series.map(x=>[x.uid,x]));assembly.forEach((inst,i)=>{const p=instancePart(inst),row=leaks.get(inst.uid),d=document.createElement('button');d.className='instance'+(inst===selectedInstance?' current':'');d.innerHTML=`<b>${String(i+1).padStart(2,'0')} · ${p.id} · ${inst.seamState}</b><span>${inst.label||p.name}</span><small>${inst.via||'origin'} · raw ${(row?.rawLeak||0).toFixed(2)} · leak ${(row?.effectiveLeak||0).toFixed(3)}${row?.repeatIndex>1?` · repeat ${row.repeatIndex}`:''}</small>`;d.onclick=()=>{selectedInstance=inst;selectedPort=instancePart(inst).ports.find(x=>!isUsed(inst,x.id))?.id||null;refresh()};el.appendChild(d)})}
...
function refresh(){renderPorts();renderParts();renderAssembly();const leak=seamTax(assembly,rules.repeatFloor??.15),raw=assembly.reduce((s,x)=>s+(x.seamTax||0),0);$('#attention').textContent=`leak ${leak.toFixed(3)} / raw ${raw.toFixed(2)}`;$('#vocab').textContent=`vocab ${coverage()}/${library.parts.length}`;renderReal()}
...
const STATION=[
  {key:'deck',root:'3020',color:71,label:'FIELD STATION DECK'},
  {key:'foundation',parent:'deck',port:'bottom-0',part:'3001',childPort:'top-0',color:72,label:'FOUNDATION · REPEATED CLUTCH'},
  {key:'cabin',parent:'deck',port:'top-0',part:'3003',childPort:'bottom-nw',color:14,label:'CABIN'},
  {key:'console',parent:'deck',port:'top-2',part:'3004',childPort:'bottom-l',color:4,label:'CONTROL CONSOLE'},
  {key:'sensor',parent:'deck',port:'top-7',part:'87087',childPort:'bottom',color:1,label:'SIDE SENSOR BLOCK'},
  {key:'sensorPanel',parent:'sensor',port:'front',part:'3023',childPort:'bottom-l',color:15,label:'SIDE-FACING SENSOR PANEL'},
  ...
  {key:'headlight',parent:'mast',port:'top',part:'4070',childPort:'bottom',color:14,label:'SNOT SENSOR HEAD'},
  ...
  {key:'sign',parent:'bracket',port:'front-l',part:'3023',childPort:'bottom-l',color:15,label:'VERTICAL SIGNAL PANEL'},
  {key:'gatewayA',parent:'console',port:'top-l',part:'3700',childPort:'bottom-l',color:4,label:'SYSTEM ↔ TECHNIC GATEWAY'},
  {key:'pinA',parent:'gatewayA',port:'hole-front',part:'2780',childPort:'a',color:0,label:'PIN COUPLER'},
  {key:'cross',parent:'pinA',port:'b',part:'6536',childPort:'pin-front',color:71,label:'90° AXIS TURN'},
  {key:'axle',parent:'cross',port:'axle-right',part:'4519',childPort:'a',color:0,label:'AXLE SENSOR BOOM'},
  {key:'bushA',parent:'axle',port:'b',part:'3713',childPort:'axis-front',color:14,label:'AXIAL STOP'},
  {key:'gatewayB',parent:'roof',port:'top-sw',part:'3700',childPort:'bottom-l',color:4,label:'SECOND STANDARD GATEWAY'},
  {key:'adapter',parent:'gatewayB',port:'hole-front',part:'43093',childPort:'pin',color:0,label:'PIN ↔ AXLE ADAPTER'},
  {key:'bushB',parent:'adapter',port:'axle',part:'3713',childPort:'axis-front',color:14,label:'END STOP'}
];
function loadPlan(plan,name){resetAssembly();demoPlan=plan;demoCursor=0;demoNodes={};$('#trace').textContent=`${name} · ${plan.length} REAL PARTS · ATTENTION SHOULD LEAK LESS AS THE GRAMMAR REPEATS`}
...
async function playPlan(plan,name){if(demoBusy)return;demoBusy=true;loadPlan(plan,name);$$('#buildActions button').forEach(b=>b.disabled=true);while(demoCursor<demoPlan.length){if(!await stepPlan())break}$$('#buildActions button').forEach(b=>b.disabled=false);demoBusy=false;if(demoCursor===demoPlan.length){const series=seamLeakSeries(assembly,rules.repeatFloor??.15),worst=[...series].sort((a,b)=>b.effectiveLeak-a.effectiveLeak)[0];$('#trace').textContent=`${name} COMPLETE · ${coverage()}/${library.parts.length} TYPES · SITUATED LEAK ${seamTax(assembly,rules.repeatFloor??.15).toFixed(3)} · WORST ${worst?.joint||'none'} ${worst?.effectiveLeak.toFixed(3)||'0'}`;flash(`GOOD APIs MAKE THE SEAMS BORING`)}}

$('#chairBtn').onclick=()=>playPlan(STATION,'FIELD STATION');
$('#operatorBtn').onclick=()=>playPlan(CHAIR,'CHAIR');
$('#stepBtn').onclick=async()=>{if(demoBusy)return;if(!demoPlan||demoCursor>=demoPlan.length)loadPlan(STATION,'FIELD STATION');await stepPlan()};
...
if(ldrawReady)setTimeout(()=>playPlan(STATION,'FIELD STATION'),420);
