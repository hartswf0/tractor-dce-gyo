...
import {actorKit,makeCostumeMeshes,makeBeardMesh,assemblyLabel} from './odyssey-actor-kit.js';
...
function renderTray(){
 ...
 if(mode==='assembly'){const k=actorKit(character),d=document.createElement('div');d.className='assembly';for(const [a,b] of [['HEAD CLASS',k.head.replaceAll('_',' ')],['BEARD',k.beard||'none'],['HAIR',k.hair||'special / none'],['COSTUME',k.costume.pattern]]){const u=document.createElement('div');u.className='unit';u.innerHTML=`<small>${a}</small><b>${String(b).toUpperCase()}</b>`;d.appendChild(u)}tray.appendChild(d);const f=document.createElement('div');f.className='frame';for(const [id,label] of [['full','FULL BODY'],['head','HEAD / MAKEUP']]){const b=document.createElement('button');b.textContent=label;if(frameMode===id)b.className='on';b.onclick=()=>{frameMode=id;fit();renderTray()};f.appendChild(b)}tray.appendChild(f);const note=document.createElement('div');note.className='foot';note.textContent='ACTUAL LDRAW BODY · explicit white sclera · Odyssey ink · beard silhouette · costume print';tray.appendChild(note);return}
