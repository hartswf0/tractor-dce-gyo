/* The Odyssey location in Film Butter: it loads, its cast stands, its camera marks frame, the walk has colliders, the forage
   ...
   NODE_PATH=<playwright> node film-readymades/test_odyssey.cjs [--out dir] [--loc odyssey-od-b12-s03] [--quick: no walk or shelf] */
...
const loc=process.argv.includes('--loc')?process.argv[process.argv.indexOf('--loc')+1]:null;
if(loc){await p.evaluate(id=>{const t=ButterFilms.records.find(r=>r.id===id).title,o=[...document.querySelectorAll('#versionSelect option')].find(o=>o.textContent.trim()===t)||[...document.querySelectorAll('#versionSelect option')].find(o=>o.textContent.includes(t));return ButterFilms.switchTo(o.value);},loc);await p.waitForFunction(id=>ButterFilms.current?.sourceId===id&&!ButterFilms.busy,loc,{timeout:240000});}
...
if(process.argv.includes('--quick')){console.log('errors',errors.length);await b.close();return;}
