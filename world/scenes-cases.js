...
/* A close-up, placed by hand: the camera d metres from the one at (x, z), toward the one at (x2, z2), swung off that line by off degrees
...
Film.SCENES['case-grocery'] = { name: 'The List', time: 'day', weather: 'clear', ground: 'flat', me: 'off', part: 'homer', set: { kind: 'stage', r: 120, seed: 2 }, marksHidden: true,
  story: { title: 'The List', description: 'Marge wrote the list; Homer carries it into the supermarket: five red apples, green bananas, black berries, extra virgin olive oil, organic flour, southwest style hash browns. Maggie rides in the cart. Ned Flanders sees the list and keeps a record of what goes in the cart. At the apples Homer runs the shopkeeper\'s three routines: the crate marked APPLES, the sample RED, the count to five. At the oil he takes the cheaper light oil and offers to change the list; Lisa tells him to change the basket (the mistake is in the performance). At the belt the receipt meets the record: the flour Ned guessed plain was organic, and Ned changes his record (the mistake is in the record). The baby scans.', location: 'the Springfield supermarket',
  ...
  shots: [
    ...
    { name: 'Homer: Flanders', ...CU(M_IN, M_NED, { off: 25 }), sec: 2.4, look: STORE,
      acts: [{ who: 'homer', from: 'in', look: 'flanders' }, { who: 'cart', from: 'cart-in', halt: true }, { who: 'bart', from: 'bart-in', look: 'homer' }, { who: 'lisa', from: 'lisa-in', look: 'homer' }, { who: 'flanders', from: 'ned-desk', look: 'homer', pose: 'stand' }],
      events: [P('homer', 'skepticism', 0.1), SAY('homer', 'Flanders.', 0.7, 0.78)] },
    { name: 'Ned: a little record', ...CU(M_NED, M_IN, { off: 25 }), sec: 2.6, look: STORE,
      acts: [{ who: 'flanders', look: 'homer', pose: 'stand' }],
      events: [P('flanders', 'joy', 0.1), SAY('flanders', 'I\'ll just keep a little record.', 0.35, 1.31)] },
    { name: 'The record begins', title: "NED'S RECORD", style: 'paper', paper: 'record', write: [0, 0.2, 1.1], sec: 2.2 },
    { name: 'The list read', title: LIST(...ALL5), style: 'paper', paper: 'list', sec: 5.0, shift: 'list',
      events: [BEAT('list', 'homer', 0, 5.0, 'the list says what should become true, not how', 'resolve'), SAY('homer', 'Apples, bananas, berries, oil, flour, hash browns.', 0.5, 3.43)] },
    { name: 'Homer: six things', ...CU(M_IN, M_LOOK, { off: -40, d: 2.8 }), sec: 3.6, look: STORE,
      acts: [{ who: 'homer', from: 'in', look: 'list-look' }, { who: 'cart', from: 'cart-in', halt: true }, { who: 'flanders', look: 'homer', pose: 'stand' }],
      events: [P('homer', 'resolve', 0.2), SAY('homer', 'Six things. How hard can it be?', 0.4, 2.16)] },
    /* the apples: the three routines, the count seen through the crate, the record takes it down */
    { name: 'Five red apples', on: 'homer', title: LIST(...ALL5), style: 'list', record: [], pos: [21.6, 3.3, -15.4], tgt: [26.0, 1.1, -10.2], lens: 50, sec: 3.4, shift: 'apples', look: STORE,
      acts: [{ who: 'homer', from: 'apples', look: 'crate' }, { who: 'cart', from: 'cart-apples', halt: true }, { who: 'lisa', from: 'lisa-apples', look: 'homer' }, { who: 'bart', from: 'bart-apples', look: 'maggie' }, { who: 'flanders', from: 'ned-apples', look: 'homer', pose: 'stand' }],
      events: [BEAT('apples', 'homer', 0, 3.4, 'the words are three routines: the crate marked APPLES, the sample RED, the count to five', 'resolve'), SAY('homer', 'Apples.', 0.5, 0.81), P('homer', 'command', 0.4), SAY('homer', 'Red.', 1.6, 0.66)] },
    { name: 'Homer counts', pos: [25.9, 1.55, -8.6], tgt: [25.75, 2.1, -11.55], lens: 32, sec: 3.6, look: STORE,
      acts: [{ who: 'homer', from: 'apples', look: 'crate' }],
      events: [SAY('homer', 'One, two, three, four, five.', 0.4, 2.11), SND('click', 0.5), SND('click', 0.92), SND('click', 1.34), SND('click', 1.76), SND('click', 2.18)] },
    { name: 'Ned: noted', ...CU(M_NEDAPP, M_APPLES, { off: -25 }), sec: 2.6, look: STORE,
      acts: [{ who: 'flanders', from: 'ned-apples', look: 'homer', pose: 'stand' }],
      events: [P('flanders', 'joy', 0.2), SAY('flanders', 'Five red apples. Noted.', 0.4, 1.39)] },
    { name: 'Apples in the record', title: "NED'S RECORD\nfive red apples", style: 'paper', paper: 'record', write: [1, 0.2, 1.2], sec: 2.0 },
    /* the oil: the wrong bottle, the list defended, the basket changed to fit the words */
    { name: 'The wrong oil', on: 'homer', title: LIST('~five red apples', '~green bananas', '~black berries', 'extra virgin olive oil', 'organic flour', 'southwest style hash browns'), style: 'list', record: ['five red apples', 'green bananas', 'black berries'], pos: [2.1, 2.7, 4.25], tgt: [2.5, 1.6, -0.75], lens: 38, sec: 4.8, shift: 'oil', look: STORE,
      acts: [{ who: 'homer', from: 'oil', look: 'shelf3' }, { who: 'cart', from: 'cart-oil2', halt: true }, { who: 'lisa', from: 'lisa-oil2', look: 'homer' }, { who: 'bart', from: 'bart-oil2', look: 'homer' }, { who: 'flanders', from: 'ned-aisle2', look: 'homer', pose: 'stand' }],
      events: [BEAT('oil', 'homer', 0, 4.8, 'a mistake in the performance: the basket is wrong, not the list', 'irony'), SAY('homer', 'Light olive oil. It\'s cheaper.', 0.3, 1.76), SND('click', 1.6), SAY('lisa', 'Dad, the list says extra virgin.', 2.3, 2.01), LOOK('homer', 'lisa', 2.3, true)] },
    { name: 'Homer: change the list', pos: [3.2, 2.4, -2.2], tgt: [2.65, 2.3, 0], lens: 30, sec: 2.6, look: STORE,
      acts: [{ who: 'homer', from: 'oil', look: 'lisa' }, { who: 'lisa', from: 'lisa-oil2', look: 'homer' }],
      events: [P('homer', 'resolve', 0.1), SAY('homer', 'So I\'ll change the list.', 0.5, 1.5)] },
    { name: 'Lisa: change the basket', pos: [1.9, 1.95, 1.0], tgt: [1.6, 1.75, -1.45], lens: 30, sec: 2.4, look: STORE,
      acts: [{ who: 'lisa', from: 'lisa-oil2', look: 'homer' }],
      events: [P('lisa', 'concern', 0.1), SAY('lisa', 'No! Change the basket.', 0.3, 1.46)] },
    { name: 'The shelf', title: LIST('~five red apples', '~green bananas', '~black berries', 'extra virgin olive oil', 'organic flour', 'southwest style hash browns'), style: 'list', on: 'homer', pos: [1.2, 1.8, 3.6], tgt: [3.5, 1.3, 1.0], lens: 32, sec: 3.6, look: STORE,
      fit: [{ at: 0.4, for: 3.0, dir: 'list', a: 'extra virgin', b: 'light goes back', say: 'the list stays right: the basket is changed to fit it' }],
      acts: [{ who: 'homer', from: 'oil', look: 'shelf3' }],
      events: [SND('click', 0.9), SND('click', 1.6)] },
    { name: 'Oil struck', title: LIST('~five red apples', '~green bananas', '~black berries', 'extra virgin olive oil', 'organic flour', 'southwest style hash browns'), style: 'paper', paper: 'list', strike: [4, 0.3, 0.6], sec: 1.6 },
    /* the freezers: caught at it */
    { name: 'Caught', on: 'homer', title: LIST('~five red apples', '~green bananas', '~black berries', '~extra virgin olive oil', '~organic flour', 'southwest style hash browns'), style: 'list', record: ['five red apples', 'green bananas', 'black berries', 'extra virgin olive oil', '?plain flour'], pos: [-24.6, 3.1, -9.6], tgt: [-27.0, 1.5, 1.2], lens: 38, sec: 4.2, shift: 'frozen', look: STORE,
      acts: [{ who: 'homer', from: 'frozen', look: 'doors' }, { who: 'cart', from: 'cart-frozen', halt: true }, { who: 'bart', from: 'bart-frozen', look: 'homer' }, { who: 'lisa', from: 'lisa-frozen', look: 'homer' }, { who: 'flanders', from: 'ned-frozen', look: 'doors', pose: 'stand' }],
      events: [BEAT('frozen', 'homer', 0, 4.2, 'the shopper turns and finds the detective', 'surprise'), SAY('homer', 'Southwest style hash browns.', 0.3, 1.93), SND('click', 1.7), SAY('flanders', 'Southwest style! Noted!', 2.5, 1.22)] },
    { name: 'Homer: Flanders?!', ...CU(M_FROZEN, M_NEDFRZ, { off: 30 }), sec: 2.0, look: STORE,
      acts: [{ who: 'homer', from: 'frozen', look: 'flanders' }, { who: 'flanders', from: 'ned-frozen', look: 'doors', pose: 'stand' }],
      events: [P('homer', 'wonder', 0.1), SAY('homer', 'Flanders?!', 0.55, 0.78)] },
    { name: 'Ned: the labels', ...CU(M_NEDFRZ, M_DOORS, { off: 20, d: 1.9, lens: 36 }), sec: 2.6, look: STORE,
      acts: [{ who: 'flanders', from: 'ned-frozen', look: 'doors', pose: 'stand' }],
      events: [SAY('flanders', 'Just reading the labels!', 0.5, 1.13)] },
    /* the belt: the receipt meets the record */
    { name: 'The belt', on: 'cart', title: LIST(...STRUCK(6)), style: 'list', record: ['five red apples', 'green bananas', 'black berries', 'extra virgin olive oil', '?plain flour', 'southwest hash browns'], pos: [-6.4, 3.6, -18.9], tgt: [-8.4, 1.2, -13.4], lens: 40, sec: 7.1, shift: 'belt', look: STORE,
      acts: [{ who: 'homer', from: 'belt', look: 'clerk' }, { who: 'cart', from: 'cart-belt', halt: true }, { who: 'bart', from: 'bart-belt', look: 'maggie' }, { who: 'lisa', from: 'lisa-belt', look: 'homer' }, { who: 'flanders', from: 'ned-belt', look: 'clerk', pose: 'stand' }, { who: 'clerk', from: 'clerk', look: 'maggie' }],
      events: [BEAT('belt', 'clerk', 0, 7.1, 'the receipt says what was bought; the record checks itself against it', 'deadpan'), SND('servo', 0.3), SND('ping', 0.95), SAY('clerk', 'Five red apples.', 1.0, 1.15), SAY('flanders', 'Check.', 2.35, 0.56), SND('ping', 2.93), SAY('clerk', 'Green bananas.', 3.11, 1.09), SAY('flanders', 'Check.', 4.4, 0.56), SND('ping', 4.9), SAY('clerk', 'Extra virgin olive oil.', 5.16, 1.67)] },
    { name: 'Ned: check-diddly-eck', ...CU(M_NEDBELT, M_CLERK, { off: 30 }), sec: 2.0, look: STORE,
      acts: [{ who: 'flanders', from: 'ned-belt', look: 'clerk', pose: 'stand' }],
      events: [P('flanders', 'joy', 0.1), SAY('flanders', 'Check-diddly-eck.', 0.3, 0.92)] },
    { name: 'Organic flour', on: 'cart', title: LIST(...STRUCK(6)), style: 'list', record: ['five red apples', 'green bananas', 'black berries', 'extra virgin olive oil', '?plain flour', 'southwest hash browns'], pos: [-6.4, 3.6, -18.9], tgt: [-8.4, 1.2, -13.4], lens: 40, sec: 1.8, shift: 'record', look: STORE,
      acts: [{ who: 'homer', from: 'belt', look: 'clerk' }, { who: 'cart', from: 'cart-belt', halt: true }, { who: 'flanders', from: 'ned-belt', look: 'clerk', pose: 'stand' }, { who: 'clerk', from: 'clerk', look: 'maggie' }],
      events: [BEAT('record', 'flanders', 0, 1.8, 'a mistake in the record: the record is wrong, not the flour; the baby is on neither', 'deadpan'), SND('ping', 0.2), SAY('clerk', 'Organic flour.', 0.25, 1.1)] },
    { name: 'Ned: I wrote plain', ...CU(M_NEDBELT, M_CLERK, { off: 30 }), sec: 2.2, look: STORE,
      acts: [{ who: 'flanders', from: 'ned-belt', look: 'clerk', pose: 'stand' }],
      events: [P('flanders', 'concern', 0.1), SAY('flanders', 'Organic? I wrote plain.', 0.4, 1.22)] },
    { name: 'The record corrected', title: "NED'S RECORD\nfive red apples\ngreen bananas\nblack berries\nextra virgin olive oil\nplain flour\nsouthwest hash browns\n+organic flour", style: 'paper', paper: 'record', strike: [5, 0.3, 0.6], write: [7, 1.2, 1.0], sec: 5.6,
      fit: [{ at: 2.4, for: 2.9, dir: 'record', a: 'organic flour', b: 'plain is crossed out', say: 'the flour stays organic: the record is changed to fit it' }],
      events: [SAY('flanders', 'My record\'s wrong, not the flour.', 0.4, 1.56)] },
    { name: 'Baby', on: 'maggie', pos: [-5.5, 1.95, -16.0], tgt: [-8.0, 1.55, -14.7], lens: 34, sec: 2.0, look: STORE,
      acts: [{ who: 'cart', from: 'cart-belt', halt: true }, { who: 'clerk', from: 'clerk', look: 'maggie' }],
      events: [SND('ping', 0.3), SAY('clerk', 'Baby.', 0.4, 0.7)] },
    { name: 'Homer: not on the list', ...CU(M_BELT, M_CLERK, { off: -25 }), sec: 2.4, look: STORE,
      acts: [{ who: 'homer', from: 'belt', look: 'clerk' }],
      events: [P('homer', 'concern', 0.05), SAY('homer', 'She\'s not on the list!', 0.3, 1.37)] },
    { name: 'Maggie on the list', title: LIST(...STRUCK(6), '+Maggie'), style: 'paper', paper: 'list', write: [7, 0.2, 0.8], sec: 1.8 },
    { name: 'Clerk: it scanned', ...CU(M_CLERK, M_CARTBELT, { off: 15 }), sec: 2.6, look: STORE,
      acts: [{ who: 'clerk', from: 'clerk', look: 'maggie' }],
      events: [P('clerk', 'weariness', 0.1), SAY('clerk', 'It scanned.', 0.4, 0.85)] },
...
/* Studs to metres: the homestead is 48 × 48 studs laid at heading 180, so world metres = (stud − 24) / 2 on both axes; it is laid a plate down
   ...
   (x −1..1, z −2.5..−2); the room x 13..34 × z 5..18 (x −5.5..5.5, z −9.5..−2.5); the porch boards z 20..28 (z −2..2.5), its posts at
   x 12, 18, 28, 34 on z 27, z 1.5..2.5 (the gap before the door x −2..2); the table x 15..20 × z 11..14 (x −4.5..−1.5, z −6.5..−4.5); the bed x 29..33 × z 5..7
   (x 2.5..5, z −9.5..−8); the path x 21..26 south from z 29 (x −1.5..1.5, z 2.5..12); the hitching rail and horse at x 4..8, z 3..4.5.
   The LEGO buttes model is not laid here: the monument set's own stepped mesas and spires stand round the horizon in haze, and through the door the flat runs to them, as in the film.
...
const SUN = { hemi: 0.55, sun: 2.1, elev: 22, azim: 225, exposure: 1.2, tone: 'aces', sunCol: 0xffc47e, fillSky: 0x7390d6, fillGround: 0xa85a36, haze: ['#d4b9a8', 40, 900], warmth: 0.75 };   /* the painter's afternoon: one low sun from the south-west for every outdoor shot, gold on whatever faces it, a cool sky fill in the shadows, a warm bounce off the red earth, and a violet haze that lays the mesas back one behind another */
const AGAINST = { hemi: 0.13, sun: 2.0, elev: 32, azim: 140, exposure: 1.3, tone: 'aces', sunCol: 0xffc88a, fillSky: 0x6d86c8, fillGround: 0x9a5634, haze: ['#d4b9a8', 40, 900], warmth: 0.8 };   /* the take, as the original lights it: the sun out over the desert facing the door, low enough to reach under the porch roof, so the boards are lit gold, whoever stands on them is a dark shape with a burning edge, and the shadows run back toward the house */
const HARD = { contrast: 1.12, sat: 1.1, sepia: 0, warm: 0.05, halation: 0.32, halBright: 1.15, clarity: 0.32, clarityRadius: 240, curve: 0.22, split: 0.38, vignette: 0.5, grain: 0.05 };   /* the doorway: the dark kept dark, the light outside glowing at its edges, the boards and the man cut clear */
/* Cut shot for shot to the film's own ending (the reference clip, 0 to 114.95 s): eight set-ups, each as long as the original's, the last one
   the long take that starts on the porch looking out, pulls back through the door into the dark room, holds the doorway while the family and
   then the young couple go in past the man, and ends when the door closes on him walking away. Here that take is three shots with no cut
   between them (a hold on the porch, the pull back, the hold inside): the camera's position carries straight across. Lars Jorgensen is Aaron,
   Mrs Jorgensen is Martha, Laurie is Lucy; Martin rides in with Ethan.
     1  0.00- 3.88  the porch, low: the old man in the rocker, the women at the edge, looking out
     2  3.88-10.47  the pool: loose horses drinking, the riders far off under the mesas
     3 10.47-16.47  the porch again: Lucy breaks and runs
     4 16.47-33.41  the flat: she runs out to Martin; Ethan rides on toward us with Debbie
     5 33.41-41.50  Martha and Aaron at the post, watching him come
     6 41.50-49.17  under the mitten: he lifts her down
     7 49.17-55.18  Mose in his rocker, the hat in the air
     8 55.18-108.50 the doorway (8a porch 55.18-68.50, 8b the pull back 68.50-75.00, 8c the hold 75.00-95.00 and, on the same camera, his walk
                    95.00-108.50; the door shuts 106.5-107.5 and the frame stays black a second, as the original's does) */
const PORCHCAM = [0.1, 2.2, -2.8], OUTAIM = [0.1, 1.3, 12], ROOMCAM = [0.1, 1.7, -8.7], ROOMAIM = [0.1, 1.7, -2.25];   /* inside, a long lens: the doorway fills the frame's height, black either side, as the original frames it */
const PORCHLOW = [-8.5, 1.1, -1.6], PORCHAIM = [0, 1.5, 1.5];   /* on the porch between the rocker and the posts, looking east: the old man in the rocker at the left, the posts and the desert at the right */
const SHADE = { hemi: 0.75, sun: 2.1, elev: 22, azim: 225, exposure: 1.25, tone: 'aces', sunCol: 0xffc47e, fillSky: 0x7390d6, fillGround: 0xa85a36, haze: ['#d4b9a8', 40, 900], warmth: 0.75 };   /* under the porch roof: the low sun reaches in under it, a little more fill for the faces */
const INTO = [[0.1, -1.4], [0.7, -3.3]];   /* through the door, then off the camera's line so the walk passes it on the side */
Film.SCENES['case-searchers'] = { name: 'The Searchers', time: 'day', weather: 'clear', ground: 'flat', me: 'off', part: 'ethan',
  grade: { contrast: 1.06, sat: 1.12, sepia: 0, warm: 0.05, halation: 0.22, clarity: 0.3, clarityRadius: 240, curve: 0.22, split: 0.35, vignette: 0.35, grain: 0.05 },   /* a painting's values: clarity so every stud and edge reads, an S for depth, the shadows cool and the lights warm */
  paintSky: { seed: 11, warmth: 0.75, cover: 16 },   /* a painted cumulus sky, lit from each shot's sun (world/paintsky.js) */
  set: { kind: 'monument', r: 200, seed: 3, corridor: [[-1, -14], [1, 12], [3, 40], [3, 70]], keepOut: [[-13, -13, 13, 13], [-24.5, 28.5, -7.5, 39.5]] },   /* the LEGO strewn on the flat stays off the homestead, the pool and the buttes */
  story: { title: 'The Searchers', description: 'The family on the porch sees riders coming in past the pool under the buttes. The girl runs out to the young man. The other man rides on to the house with the girl the Comanche took, and lifts her down. The family takes her in, and the young couple go in after them. He stays on the porch, and walks back into the desert, and the door shuts.', location: 'a homestead under the buttes',
    entities: [{ id: 'ethan', type: 'character', name: 'Ethan', traits: ['the hat', 'five years searching', 'the door he does not go through'] }, { id: 'debbie', type: 'character', name: 'Debbie', traits: ['taken', 'brought home'] }, { id: 'martha', type: 'character', name: 'Martha', traits: ['the porch', 'the one who waited'] }, { id: 'aaron', type: 'character', name: 'Aaron', traits: ['the rocker', 'the house'] }, { id: 'martin', type: 'character', name: 'Martin', traits: ['rode with him'] }, { id: 'lucy', type: 'character', name: 'Lucy', traits: ['runs out to Martin'] }, { id: 'mose', type: 'character', name: 'Mose', traits: ['the rocker', 'the hat'] }, { id: 'door', type: 'location', name: 'the door', traits: ['the dark room', 'the bright desert'] }],
    goals: ['bring Debbie home'], obstacles: ['the man who brings her is not a man for houses'], shifts: [{ id: 'porch', name: 'The porch' }, { id: 'sighting', name: 'The pool' }, { id: 'run', name: 'Lucy runs' }, { id: 'approach', name: 'The approach' }, { id: 'recognition', name: 'Martha' }, { id: 'lift', name: 'Down from the horse' }, { id: 'mose', name: 'Mose' }, { id: 'home', name: 'Home' }, { id: 'inside', name: 'Inside' }, { id: 'leaving', name: 'The leaving' }], why: 'the one who brings them home cannot come in', direction: 'restraint' },
  donors: [{ name: 'the homestead', set: 'homestead', x: 0, z: 0, heading: 180, scale: 1, y: -0.18 }, { name: 'the waterhole', set: 'waterhole', x: -16, z: 34, heading: 180, scale: 1, y: -0.12 }],
  marksHidden: true,
  marks: { 'porch-m': [-3.0, 1.6, 0, 165], 'porch-l': [-1.0, -0.2, 0, 175], 'run0': [-1.2, 8.0, 0, 180], run: [-1.6, 24.5], 'post-m': [-4.4, 1.5, 0, 172], 'post-a': [-0.7, 1.5, 0, 188],
    'far-e': [-7.5, 58, 0, 0], 'far-t': [-4.8, 59.5, 0, 0], 'flat-e': [0.8, 40, 0, 0], 'flat-t': [-2.4, 41, 0, 0], meet: [-2.0, 27], 'ride-in': [1.1, 18], lift: [5.5, 16.0, 0, 247], 'e-lift': [4.9, 14.5, 0, 157], 'd-lift': [4.0, 14.0, 0, 170],
    'e-path': [0.4, 7.2, 0, 0], 'd-path': [1.3, 7.6, 0, 0], 'e-step': [0.7, 3.4], 'd-step': [-0.3, 3.0], 'a-edge': [1.9, 2.4, 0, 190], 'm-edge': [-1.5, 2.4, 0, 170],
    't-yard': [-2.2, 15.5, 0, 0], 'l-yard': [-1.4, 15.2, 0, 0], 't-near': [-0.8, 5.0], 'l-near': [-0.1, 5.3], 'e-door': [-0.7, 1.2, 0, 0], 'e-in': [-0.2, 1.7, 0, 0],
    'room-d': [3.4, -6.4], 'room-m': [1.8, -8.6], 'room-a': [4.8, -7.4], 'room-l': [3.6, -8.9], 'room-t': [5.0, -5.4], desert: [1.6, 30], pasture: [-17.0, 12.0] },
  actors: [
    { name: 'martha', figure: 'martha', label: 'Martha', x: -3.0, z: 1.6, heading: 165 },
    { name: 'aaron', figure: 'aaron', label: 'Aaron', x: -4.5, z: -1.0, heading: 180, pose: 'sit', sitLift: 10 },   /* Lars Jorgensen's rocker, at the west end of the porch */
    { name: 'lucy', figure: 'lucy', label: 'Lucy', x: -1.0, z: -0.2, heading: 175 },
    { name: 'mose', figure: 'mose', label: 'Mose Harper', x: 4.0, z: -1.0, heading: 180, pose: 'sit', sitLift: 10 },
    { name: 'ethan', figure: 'ethan', label: 'Ethan', x: -7.5, z: 58, heading: 0 },
    { name: 'debbie', figure: 'debbie', label: 'Debbie', x: -7.0, z: 58.6, heading: 0 },
    { name: 'martin', figure: 'martin', label: 'Martin', x: -4.8, z: 59.5, heading: 0 },
    { name: 'ethan-horse', kind: 'horse', len: 7, col: 19, saddle: 0, label: "Ethan's horse", x: -7.5, z: 58, heading: 0 },
    { name: 'martin-horse', kind: 'horse', len: 7, col: 0, saddle: 70, label: "Martin's horse", x: -4.8, z: 59.5, heading: 0 }],   /* the loose horses at the pool are the waterhole model's own three */
  shots: [
    { name: 'The porch', on: 'martha', pos: PORCHLOW, tgt: PORCHAIM, lens: 44, sec: 3.88, shift: 'porch', look: SHADE, score: 'file:../music/searchers-score.ogg',
      acts: [{ who: 'ethan', ride: 'ethan-horse' }, { who: 'debbie', ride: 'ethan-horse' }, { who: 'martin', ride: 'martin-horse' }, { who: 'martha', from: 'porch-m', look: 'ethan' }, { who: 'lucy', from: 'porch-l', look: 'martin' }],
      events: [BEAT('porch', 'martha', 0, 3.88, 'the family on the porch, looking out', 'wonder'), LOOK('martha', 'ethan', 0.2, true), LOOK('lucy', 'martin', 0.3, true), LOOK('aaron', 'ethan', 0.5, true)] },
    { name: 'The pool', on: 'ethan', pos: [-16, 2.0, 17], tgt: [-13, 1.5, 55], lens: 40, sec: 6.59, shift: 'sighting', look: SUN,
      acts: [{ who: 'ethan-horse', from: 'far-e', to: 'flat-e', speed: 0.62 }, { who: 'martin-horse', from: 'far-t', to: 'flat-t', speed: 0.62 }],
      events: [BEAT('sighting', 'ethan', 0, 6.59, 'riders under the mesas, past the loose horses at the pool', 'wonder'), { ...SND('snort', 1.2), k: 0.35 }, { ...SND('hooves', 3.0), k: 0.25 }, { ...SND('whinny', 5.2), k: 0.3 }] },
    { name: 'Lucy runs', on: 'lucy', pos: PORCHLOW, tgt: PORCHAIM, lens: 44, sec: 6.01, shift: 'run', look: SHADE,
      acts: [{ who: 'lucy', to: 'run', run: true, at: 3.3 }, { who: 'martha', look: 'ethan' }],
      events: [BEAT('run', 'lucy', 0, 6.01, 'she knows the second rider, and breaks for him', 'joy'), P('lucy', 'joy', 2.6), P('martha', 'wonder', 1.0), SND('footstep', 3.4), SND('footstep', 3.7), SND('footstep', 4.0)] },
    { name: 'The approach', on: 'ethan', pos: [-2.2, 1.9, 3.4], tgt: [-0.6, 1.8, 40], lens: 30, sec: 16.93, shift: 'approach', look: SUN,
      acts: [{ who: 'lucy', from: 'run0', to: 'meet', run: true }, { who: 'ethan-horse', from: 'flat-e', to: 'ride-in', speed: 0.7 }, { who: 'martin-horse', from: 'flat-t', to: 'meet', speed: 0.5 }, { who: 'martin', leave: true, to: 'meet', at: 9.5 }],
      events: [BEAT('approach', 'ethan', 0, 16.93, 'she reaches Martin; Ethan rides on toward the house with the girl', 'weariness'), SND('footstep', 0.3), SND('footstep', 0.6), SND('footstep', 0.9), { ...SND('hooves', 4.0), k: 0.3 }, { ...SND('hooves', 8.0), k: 0.45 }, { ...SND('whinny', 10.4), k: 0.4 }, { ...SND('hooves', 12.0), k: 0.6 }, { ...SND('hooves', 14.0), k: 0.7 }, { ...SND('snort', 15.6), k: 0.6 }, P('ethan', 'weariness', 11.0)] },
    { name: 'Martha', on: 'martha', pos: [-2.3, 1.7, 8.8], tgt: [-2.5, 1.7, 1.7], lens: 30, sec: 8.09, shift: 'recognition', look: SUN,
      acts: [{ who: 'martha', from: 'post-m', look: 'ethan' }, { who: 'aaron', from: 'post-a', pose: 'stand', look: 'ethan' }, { who: 'ethan', leave: true }],   /* he gets down off camera, so the next shot finds him on his mark by the horse */
      events: [BEAT('recognition', 'martha', 0, 8.09, 'she sees who he has with him', 'recognition'), LOOK('martha', 'ethan', 0.2, true), LOOK('aaron', 'ethan', 0.4, true), P('martha', 'recognition', 1.2), SND('breath', 2.6), P('aaron', 'joy', 4.2), SET('martha', 'head.yaw', -0.5, 5.6, 0.8)] },
    { name: 'Down from the horse', on: 'debbie', pos: [-0.6, 0.9, 4.0], tgt: [9, 2.4, 28], lens: 30, sec: 7.67, shift: 'lift', look: SUN,
      acts: [{ who: 'ethan-horse', from: 'lift', halt: true }, { who: 'ethan', from: 'e-lift', look: 'debbie' }, { who: 'debbie', leave: true, to: 'd-lift', walk: true, at: 2.6 }],
      events: [BEAT('lift', 'ethan', 0, 7.67, 'he lifts her down from the saddle', 'tenderness'), SET('ethan', 'arm.R.pitch', -1.9, 0.8, 0.6), SET('ethan', 'arm.L.pitch', -1.9, 0.8, 0.6), SET('ethan', 'arm.R.pitch', -0.9, 3.0, 0.8), SET('ethan', 'arm.L.pitch', -0.9, 3.0, 0.8), { ...SND('snort', 1.4), k: 0.5 }, SND('footstep', 3.1), P('debbie', 'guarded', 3.6)] },
    { name: 'Mose', on: 'mose', pos: [3.5, 1.55, 4.2], tgt: [4.0, 1.25, -1.1], lens: 34, sec: 6.01, shift: 'mose', look: SHADE,
      acts: [{ who: 'mose', look: 'ethan' }],
      events: [BEAT('mose', 'mose', 0, 6.01, 'the old man in his rocker, his hat off to them', 'joy'), P('mose', 'joy', 0.3), SET('mose', 'arm.R.pitch', -2.7, 0.5, 0.5), SET('mose', 'arm.R.pitch', -2.2, 1.3, 0.4), SET('mose', 'arm.R.pitch', -2.8, 2.0, 0.4), SET('mose', 'arm.R.pitch', -2.2, 2.7, 0.4), SET('mose', 'arm.R.pitch', -2.8, 3.4, 0.4), SET('mose', 'arm.R.pitch', 0, 5.0, 0.8)] },
    { name: 'Home', on: 'debbie', pos: PORCHCAM, tgt: OUTAIM, lens: 50, sec: 13.32, shift: 'home', look: AGAINST, grade: HARD,
      acts: [{ who: 'ethan', from: 'e-path', to: 'e-step', walk: true, at: 0.2 }, { who: 'debbie', from: 'd-path', to: 'd-step', walk: true, at: 0.4 }, { who: 'aaron', from: 'a-edge', look: 'debbie' }, { who: 'martha', from: 'm-edge', look: 'debbie' }, { who: 'lucy', from: 'l-yard', to: 'l-near', walk: true, at: 1.5 }, { who: 'martin', from: 't-yard', to: 't-near', walk: true, at: 1.2 }, { who: 'ethan-horse', from: 'lift', to: 'pasture', speed: 0.3, at: 2.0 }, { who: 'martin-horse', to: 'pasture', speed: 0.3, at: 1.0 }],
      events: [BEAT('home', 'debbie', 0, 13.32, 'he brings her up to the porch; the two who waited meet her at the edge', 'tenderness'), SND('footstep', 0.8), SND('footstep', 1.4), SND('footstep', 2.0), SND('footstep', 2.6), LOOK('martha', 'debbie', 0.4, true), P('martha', 'tenderness', 3.8), LOOK('aaron', 'debbie', 0.6, true)] },
    { name: 'Home, in', on: 'martha', pos: PORCHCAM, pos2: ROOMCAM, tgt: OUTAIM, tgt2: ROOMAIM, lens: 50, lens2: 27, sec: 6.5, shift: 'home', look: AGAINST, grade: HARD,
      acts: [{ who: 'martha', to: 'room-m', via: INTO, walk: true, at: 1.6 }, { who: 'debbie', to: 'room-d', via: INTO, walk: true, at: 1.8 }, { who: 'aaron', to: 'room-a', via: INTO, walk: true, at: 2.3 }, { who: 'ethan', to: 'e-door', walk: true, at: 5.0 }, { who: 'lucy', to: 'l-near', walk: true }, { who: 'martin', to: 't-near', walk: true }],
      events: [SND('footstep', 0.6), SND('footstep', 1.1), SND('footstep', 1.6), SND('footstep', 2.1), SND('footstep', 2.6), SND('footstep', 3.1)] },
    { name: 'The door', on: 'ethan', pos: ROOMCAM, tgt: ROOMAIM, lens: 27, sec: 20.0, shift: 'inside', look: AGAINST, grade: HARD,
      acts: [{ who: 'ethan', to: 'e-in', walk: true, at: 10.5, look: 'room-m' }, { who: 'lucy', to: 'room-l', via: [[0.5, 1.6], [0.4, -1.4], [0.9, -3.3]], walk: true, at: 1.5 }, { who: 'martin', to: 'room-t', via: [[0.7, 1.4], [0.4, -1.4], [0.9, -3.3]], walk: true, at: 2.7 }],
      events: [BEAT('inside', 'ethan', 0, 20.0, 'he stands aside in the door; the young couple go in past him', 'restraint'), SND('footstep', 3.7), SND('footstep', 4.3), SND('footstep', 5.3), SND('footstep', 5.9), LOOK('ethan', 'room-m', 10.5, true), P('ethan', 'weariness', 11.5), SET('ethan', 'arm.L.pitch', -1.1, 13.0, 0.8), SET('ethan', 'arm.R.pitch', -0.35, 13.2, 0.8), SET('ethan', 'arm.L.pitch', 0, 19.0, 0.6), SET('ethan', 'arm.R.pitch', 0, 19.0, 0.6)] },
    { name: 'The leaving', on: 'ethan', pos: ROOMCAM, tgt: ROOMAIM, lens: 27, sec: 13.5, shift: 'leaving', look: AGAINST, grade: HARD, close: [11.5, 1.0],   /* the same camera as the shot before: no cut, the take goes on */
      acts: [{ who: 'ethan', to: 'desert', walk: true, at: 0.2 }],
      events: [BEAT('leaving', 'ethan', 0, 13.5, 'he turns, and walks back into the desert; the door shuts', 'restraint'), SND('footstep', 0.6), SND('footstep', 1.2), SND('footstep', 1.8), { ...SND('whinny', 6.0), k: 0.25 }, { ...SND('whoosh', 11.5), k: 0.5 }] },
    { title: 'THE SEARCHERS', style: 'card', sec: 3.0, name: 'the card' }] };   /* the score's last chord rings out over the card */
...
Film.CASES = [
  { key: 'case-grocery', name: 'The List', source: 'world/models/supermarket.js', set: 'supermarket', model: 'supermarket', size: '62 x 47.5 m', cast: ['homer', 'maggie', 'flanders', 'bart', 'lisa', 'citizen'], status: 'program', beats: ['the store', 'the list', 'five red apples', 'the wrong oil', 'caught', 'the belt', 'the record'], play: 'cinerium.html?film=case-grocery&shop' },
  ...
  { key: 'case-searchers', name: 'The Searchers', source: 'world/models/homestead.js', set: 'homestead', model: 'homestead', models: ['homestead', 'waterhole'], size: '24 x 24 m', cast: ['ethan', 'martha', 'aaron', 'debbie', 'martin', 'lucy', 'mose'], status: 'program', beats: ['the porch', 'the pool', 'Lucy runs', 'the approach', 'Martha', 'down from the horse', 'Mose', 'home', 'the door', 'the leaving'] },
