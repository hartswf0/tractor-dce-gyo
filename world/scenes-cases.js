...
Film.SCENES['case-grocery'] = { name: 'The Checkout', time: 'day', weather: 'clear', ground: 'flat', me: 'off', part: 'marge',
  ...
  shots: [
    ...
    { name: 'The queue', on: 'the lane', pos: [-6.5, 2.6, 6.5], tgt: [0.5, 1.4, 0.8], lens: 40, sec: 6, shift: 'queue', handheld: 0.2,
    ...
    { name: 'The belt', on: 'maggie', pos: [-2.2, 1.6, 4.4], tgt: [0.2, 1.3, 1.8], lens: 34, sec: 6, shift: 'belt',
    ...
    { name: 'The scan', on: 'clerk', pos: [3.6, 1.9, 2.6], tgt: [1.8, 1.4, 0.2], lens: 36, sec: 6, shift: 'scan', handheld: 0.3,
    ...
    { name: 'The price', on: 'the lane', pos: [1.2, 2.0, 5.2], tgt: [2.0, 1.6, -0.3], lens: 30, sec: 5, shift: 'price',
    ...
    { name: 'The bag', on: 'marge', pos: [-2.4, 1.7, 3.4], tgt: [-1.2, 1.7, 2.2], lens: 30, sec: 5, shift: 'bag', handheld: 0.4,
    ...
    { name: 'The fix', on: 'the lane', pos: [-5.5, 2.4, 6.0], tgt: [0.8, 1.4, 0.6], lens: 38, sec: 7, shift: 'fix',
...
/* ── 2. The forest skirmish: Ewoks against robots (ewoks.mpd), the set's own six cameras kept ── */
const EW = { wide: { pos: [0, 12.5, -30], tgt: [0, 2.5, 0], lens: 35 }, ewoks: { pos: [-16.3, 10, -7.5], tgt: [-16.3, 4, -0.6], lens: 50 }, robots: { pos: [16.3, 10, -2.5], tgt: [13.8, 4, 0], lens: 50 }, middle: { pos: [0, 11.3, 5], tgt: [0, 2.5, 0], lens: 45 }, train: { pos: [32.5, 8.8, -2.5], tgt: [13.8, 2.5, -2.5], lens: 55 }, aerial: { pos: [0, 38.8, 0.1], tgt: [0, 0, 0], lens: 55 } };
Film.SCENES['case-ewoks'] = { name: 'The Forest Skirmish', time: 'day', weather: 'clear', ground: 'flat', me: 'off', part: 'ewok-1',
  ...
  marks: { 'ewok-cover': [-14, 0, 6], 'robot-cover': [14, 0, 7], middle: [0, 0, 14], 'ewok-rush': [-5, 0, 2], 'robot-rush': [5, 0, 1], track: [13.8, -2.5, 4], back: [-18, 3, 25] },
  actors: [
  ...
  shots: [
    ...
    { name: 'Skirmish wide', on: 'the clearing', pos: EW.wide.pos, tgt: EW.wide.tgt, lens: EW.wide.lens, sec: 6, shift: 'lines', events: [BEAT('lines', 'ewok-1', 0, 6, 'two lines, a log between them, nobody moving first', 'guarded'), SND('chirp', 1.0), SND('chirp', 3.2)] },
    { name: 'Ewok line', on: 'ewok-1', pos: EW.ewoks.pos, tgt: EW.ewoks.tgt, lens: EW.ewoks.lens, sec: 4, shift: 'lines', events: [P('ewok-1', 'guarded', 0.3), P('ewok-2', 'listen', 0.5), LOOK('ewok-1', 'robot-1', 0.4, true), SAY('ewok-1', 'Yub nub.', 1.6, 1.0)] },
    { name: 'Robot line', on: 'robot-1', pos: EW.robots.pos, tgt: EW.robots.tgt, lens: EW.robots.lens, sec: 4, shift: 'intrusion', acts: [{ who: 'robot-1', to: 'robot-rush', walk: true }], events: [BEAT('intrusion', 'robot-1', 0, 4, 'one robot steps over the log', 'resolve'), SND('servo', 0.4), SND('servo', 1.6), SND('servo', 2.8)] },
    { name: "No man's land", on: 'middle', pos: EW.middle.pos, tgt: EW.middle.tgt, lens: EW.middle.lens, sec: 6, shift: 'ambush', handheld: 0.5,
    ...
    { name: 'Train vector', on: 'track', pos: EW.train.pos, tgt: EW.train.tgt, lens: EW.train.lens, sec: 5, shift: 'train', events: [BEAT('train', 'robot-2', 0, 5, 'the train comes down the crease and both sides take it for the enemy', 'fear'), SND('static', 0.3), SND('whoosh', 1.4), SND('boom', 2.6), P('robot-2', 'fear', 1.6), P('ewok-3', 'fear', 1.8), { what: 'SHAKE', at: 2.6 }] },
    { name: 'The reversal', on: 'the clearing', pos: [0, 6, 14], tgt: [0, 2, -2], lens: 40, sec: 6, shift: 'reversal',
      acts: [{ who: 'robot-1', to: 'robot-cover', walk: true }, { who: 'robot-2', to: 'robot-cover', walk: true }, { who: 'robot-3', to: 'robot-cover', walk: true }, { who: 'ewok-1', to: 'ewok-cover', walk: true }, { who: 'ewok-2', to: 'ewok-cover', walk: true }],
    ...
    { name: 'Aerial map', on: 'the clearing', pos: EW.aerial.pos, tgt: EW.aerial.tgt, lens: EW.aerial.lens, sec: 5, shift: 'aerial', events: [SND('whoosh', 0.8), SND('whoosh', 2.6)] },
    { score: 'end', title: 'THE FOREST SKIRMISH\nA CASE TO REHEARSE', style: 'card', sec: 3, name: 'the card' }] };

/* ── 3. The cave: Plato's Republic VII on the four-zone construct (platos-cave.mpd) ── */
Film.SCENES['case-plato'] = { name: "The Cave", time: 'night', weather: 'clear', ground: 'flat', me: 'off', part: 'freed',
  ...
  donors: [{ name: 'the cave', set: 'platos-cave', x: 0, z: 0, heading: 0, scale: 2.5 }],   /* the captives' row at z -5 facing the screen at z -12.5; the parapet at z 5..7.5 six metres up; the fire at z 13.5; the ascent to z 27 */
  marks: { row: [0, -4, 0], screen: [0, -11, 15], parapet: [0, 6.2, 4], fire: [0, 13, 14], foot: [0, 18, 25], top: [0, 26, 14], side: [-4, -3, 2] },
  actors: [
    ...
    { name: 'puppeteer-1', figure: 'hauler', label: 'a puppeteer', x: -2, z: 6.2, heading: 180 }, { name: 'puppeteer-2', figure: 'hauler', label: 'a puppeteer', x: 2, z: 6.2, heading: 180 }],   /* the figures stand on the floor behind the row; the parapet is the set's geometry above them */
  shots: [
    { score: 'dread', title: 'THE CAVE', style: 'card', sec: 2, events: [SAY('narrator', 'The cave. Republic, book seven.', 0.3, 2.0)] },
    { name: 'The shadows', on: 'screen', pos: [0, 2.2, 1.5], tgt: [0, 2.0, -12], lens: 44, sec: 7, shift: 'shadows', look: { hemi: 0.1, sun: 0, exposure: 0.9 }, lamp: { at: [0, 5, 12], color: 0xff7a2a, intensity: 8, distance: 40 },
      events: [BEAT('shadows', 'freed', 0, 7, 'shapes cross the wall; the captives name them', 'wonder'), SND('static', 0.5), SAY('narrator', 'They have been here since childhood, and see only the shadows.', 1.2, 4.2), P('captive-1', 'listen', 0.4), P('captive-2', 'listen', 0.6)] },
    { name: 'The puppeteers', on: 'parapet', pos: [-6.5, 3.4, 10.5], tgt: [0, 1.7, 6.2], lens: 38, sec: 6, shift: 'puppeteers', look: { hemi: 0.1, sun: 0, exposure: 0.9 }, lamp: { at: [0, 5, 12], color: 0xff7a2a, intensity: 8, distance: 40 },
      events: [BEAT('puppeteers', 'puppeteer-1', 0, 6, 'behind the wall, the people who carry the shapes', 'deadpan'), SET('puppeteer-1', 'arm.R.pitch', -2.8, 0.6, 0.8), SET('puppeteer-2', 'arm.L.pitch', -2.8, 1.0, 0.8), SND('servo', 1.2), SND('servo', 3.6), SAY('narrator', 'Between the fire and the captives runs a road, and along it a low wall, like the screen of a puppet show.', 0.8, 4.8)] },
    { name: 'The turn', on: 'freed', pos: [-3.2, 1.9, -2.6], tgt: [0, 1.6, -5], lens: 32, sec: 7, shift: 'turn', look: { hemi: 0.12, sun: 0, exposure: 0.95 },
      events: [BEAT('turn', 'freed', 0, 7, 'loosed, he turns his head and sees the fire', 'recognition'), SET('freed', 'head.yaw', 2.4, 1.0, 1.4), SET('freed', 'torso.twist', 0.6, 1.4, 1.2), P('freed', 'wonder', 2.4), LOOK('freed', 'fire', 2.6, true), SND('breath', 3.0), SAY('narrator', 'Suppose one of them is freed, and made to turn his head.', 0.6, 3.6)] },
    { name: 'The climb', on: 'freed', pos: [6, 12, 20], tgt: [0, 10, 20], lens: 40, sec: 8, shift: 'climb',
      acts: [{ who: 'freed', to: 'foot', walk: true }],
      events: [BEAT('climb', 'freed', 0, 8, 'the rough ascent toward a light that hurts', 'resolve'), P('freed', 'resolve', 0.4), SND('footstep', 1.0), SND('footstep', 2.0), SND('footstep', 3.0), SND('breath', 5.0), SAY('narrator', 'Dragged up the steep way, he would be pained and dazzled.', 2.0, 3.8)] },
    { name: 'The sun', on: 'top', pos: [0, 22, 34], tgt: [0, 20, 24], lens: 46, sec: 6, shift: 'sun', set: { kind: 'desert', r: 40, seed: 5, time: 'dawn' }, look: { hemi: 0.5, sun: 1.2, elev: 12, azim: 180, exposure: 1.0 },
      acts: [{ who: 'freed', to: 'top', walk: true }],
    ...
    { name: 'The return', on: 'row', pos: [4.5, 2.0, -1.5], tgt: [0, 1.5, -5], lens: 36, sec: 7, shift: 'return', look: { hemi: 0.1, sun: 0, exposure: 0.9 },
      acts: [{ who: 'freed', to: 'side', walk: true }],
      events: [BEAT('return', 'freed', 0, 7, 'he comes back down to tell them, and they laugh at his ruined eyes', 'hurt'), P('freed', 'appeal', 1.0), P('captive-1', 'contempt', 3.0), P('captive-2', 'skepticism', 3.4), SAY('narrator', 'Would they not say his eyes were ruined by going up, and that it was not worth even trying?', 2.0, 4.6), P('freed', 'hurt', 5.4)] },
