...
Film.SCENES['case-grocery'] = { name: 'The List', time: 'day', weather: 'clear', ground: 'flat', me: 'off', part: 'homer', set: { kind: 'stage', r: 120, seed: 2 }, marksHidden: true,
  story: { title: 'The List', description: 'Homer pushes a cart into a warehouse store with three words in his hand: milk, bread, eggs. Maggie rides in the cart. The store supplies the rest: the racks, the aisle, the cooler. Bart adds fireworks. There are no eggs. At the checkout the procedure scans whatever is at the belt, and the baby is at the belt.', location: 'a Springfield warehouse store',
    entities: [{ id: 'homer', type: 'character', name: 'Homer', traits: ['the list', 'the cart'] }, { id: 'maggie', type: 'character', name: 'Maggie', traits: ['in the cart', 'not on the list'] }, { id: 'bart', type: 'character', name: 'Bart', traits: ['the goal found on the way'] }, { id: 'lisa', type: 'character', name: 'Lisa', traits: ['reads the list'] }, { id: 'clerk', type: 'character', name: 'the clerk', traits: ['the scanner', 'the procedure'] }, { id: 'list', type: 'object', name: 'the list', traits: ['milk', 'bread', 'eggs'] }],
    goals: ['make the list true'], obstacles: ['no eggs', 'a procedure that acquires everything at the belt'], shifts: [{ id: 'store', name: 'The store' }, { id: 'list', name: 'The list' }, { id: 'aisle', name: 'The aisle' }, { id: 'dairy', name: 'No eggs' }, { id: 'belt', name: 'The belt' }, { id: 'scan', name: 'It scanned' }], why: 'the list says what should be true; the belt does not know what the list meant', direction: 'deadpan' },
  donors: [{ name: 'the store', set: 'supermarket', x: 0, z: 0, heading: 180, scale: 1, y: -0.18 }],
  /* Aisle six's floor is x 1.5..4.5 between the racks at x 1..1.5 and 4.5..5 (z −4..9.5); the back aisle z 10..13.5; the dairy glass at z 14. */
  marks: { street: [-1.2, -22], in: [-0.3, -12.2], 'aisle-n': [3.0, -6.2], 'aisle-s': [3.0, 5.0], dairy: [3.0, 11.6], lane: [3.9, -10.6], clerk: [8.4, -11.8], 'bart-start': [2.2, 11.2] },
  actors: [
    { name: 'homer', figure: 'homer', label: 'Homer', x: -1.2, z: -22, heading: 180 },
  ...
  shots: [
    ...
    { name: 'The store', on: 'homer', title: LIST('milk', 'bread', 'eggs'), style: 'list', pos: [9, 3.4, -36], pos2: [7, 3.0, -32], tgt: [0, 4.2, -15], lens: 45, sec: 7.84, shift: 'store', look: STORE,
      acts: [{ who: 'homer', to: 'in', walk: true, at: 0.5 }, { who: 'cart', lead: 'homer', ahead: 2.3 }, { who: 'lisa', follow: 'homer', behind: 1.4, walk: true }],
      events: [BEAT('store', 'homer', 0, 7.84, 'three words go into a building the size of a field', 'neutral'), SND('footstep', 0.9), SND('footstep', 1.4), SND('footstep', 1.9), SND('clatter', 2.2), SND('whoosh', 4.6)] },
    { name: 'The list', on: 'homer', title: LIST('milk', 'bread', 'eggs'), style: 'list', pos: [3.6, 2.4, -11.4], tgt: [-0.4, 1.8, -11.2], lens: 36, sec: 3.95, shift: 'list', look: STORE,
      acts: [{ who: 'cart', lead: 'homer', ahead: 2.3 }, { who: 'lisa', look: 'homer' }],
      events: [BEAT('list', 'homer', 0, 3.95, 'the list says what should become true, not how', 'resolve'), P('homer', 'resolve', 0.2), SAY('homer', 'Milk. Bread. Eggs. Simple.', 0.4, 2.4), P('lisa', 'skepticism', 2.6)] },
    { name: 'The aisle', on: 'homer', title: LIST('milk', 'bread', 'eggs'), style: 'list', pos: [3.2, 2.3, 13.0], tgt: [3.0, 2.2, -5], lens: 36, sec: 7.89, shift: 'aisle', look: STORE,
      acts: [{ who: 'homer', to: 'aisle-s', via: [[0.2, -8.4], [3.0, -6.2]], walk: true }, { who: 'cart', lead: 'homer', ahead: 2.3 }, { who: 'lisa', follow: 'homer', behind: 1.4, walk: true }, { who: 'bart', to: [2.4, 2.4], at: 2.2 }],
      events: [BEAT('aisle', 'homer', 0, 7.89, 'the route is the store\'s, and a goal turns up on the way', 'irony'), SND('footstep', 0.5), SND('footstep', 1.0), SND('footstep', 1.5), SND('footstep', 2.0), SAY('bart', 'Dad! Fireworks!', 3.4, 1.4), LOOK('homer', 'bart', 4.4, true), SAY('homer', 'Is it on the list?', 4.9, 1.4), SAY('bart', 'It is now.', 6.4, 1.1), P('bart', 'irony', 6.2)] },
    { name: 'No eggs', on: 'homer', title: LIST('~milk', '~bread', '?eggs', '+fireworks'), style: 'list', pos: [-2.6, 2.2, 11.3], tgt: [3.2, 1.5, 12.9], lens: 38, sec: 5.91, shift: 'dairy', look: STORE,
      acts: [{ who: 'homer', to: 'dairy', walk: true }, { who: 'cart', lead: 'homer', ahead: 2.1 }, { who: 'lisa', follow: 'homer', behind: 1.4, walk: true }, { who: 'bart', follow: 'homer', behind: 2.2 }],
      events: [BEAT('dairy', 'homer', 0, 5.91, 'the world resists: the list is corrected, not the world', 'concern'), P('homer', 'skepticism', 2.2), SND('click', 2.6), SAY('homer', 'No eggs.', 3.0, 1.0), SAY('lisa', 'Cross it out, Dad.', 4.2, 1.4)] },
    { name: 'The belt', on: 'cart', title: LIST('~milk', '~bread', '?eggs', '+fireworks'), style: 'list', pos: [1.6, 2.0, -14.0], tgt: [5.4, 1.0, -11.0], lens: 45,   /* from the north-west, by the doors: the cart and Maggie in front, the belt and the clerk behind */ sec: 7.89, shift: 'belt', look: STORE,
      acts: [{ who: 'homer', to: 'lane', via: [[3.0, -6.2], [3.4, -8.6]] }, { who: 'cart', lead: 'homer', ahead: 2.1 }, { who: 'lisa', follow: 'homer', behind: 1.6 }, { who: 'bart', follow: 'homer', behind: 2.4 }, { who: 'clerk', look: 'cart' }],
      events: [BEAT('belt', 'clerk', 0, 7.89, 'the procedure acquires whatever is at the belt', 'deadpan'), SND('servo', 0.3), SND('servo', 1.5), SND('ping', 3.45), SAY('clerk', 'Milk.', 3.53, 0.6), SND('ping', 4.44), SAY('clerk', 'Bread.', 4.52, 0.6), SND('ping', 5.42), SAY('clerk', 'Fireworks.', 5.5, 0.8), SND('ping', 6.41), SAY('clerk', 'Baby.', 6.49, 0.6), P('clerk', 'neutral', 0.2), P('homer', 'concern', 7.0)] },
    { name: 'It scanned', on: 'homer', title: LIST('~milk', '~bread', '?eggs', '+fireworks', '+Maggie'), style: 'list', pos: [0.4, 2.3, -9.0], tgt: [5.0, 1.4, -12.4], lens: 42, sec: 7.89, shift: 'scan', look: STORE,
      acts: [{ who: 'homer', look: 'clerk' }, { who: 'cart', lead: 'homer', ahead: 2.1 }, { who: 'lisa', to: [2.6, -9.4], walk: true }, { who: 'bart', to: [1.4, -12.4], walk: true }],   /* the children step aside to the west, out of the line from the camera to Homer */
      events: [BEAT('scan', 'homer', 0, 7.89, 'the list did not say: not the baby', 'deadpan'), SAY('homer', 'She is not on the list!', 0.5, 1.8), P('clerk', 'shrug', 2.4), SAY('clerk', 'It scanned.', 2.6, 1.0), P('bart', 'joy', 3.8), SAY('bart', 'Can we keep the fireworks?', 4.0, 1.8)] },
...
Film.CASES = [
  { key: 'case-grocery', name: 'The List', source: 'world/models/supermarket.js', set: 'supermarket', model: 'supermarket', size: '48 x 36 m', cast: ['homer', 'maggie', 'bart', 'lisa', 'citizen'], status: 'program', beats: ['the store', 'the list', 'the aisle', 'no eggs', 'the belt', 'it scanned'] },
