...
function vehicleVerb(it) { const k = Drive.KINDS[Drive.kindOf(it)]; return k.fly ? 'Fly' : k.walker ? 'Board' : 'Drive'; }
...
function boardVehicle(it) {
  ...
  W.veh = V; W.mode = 'ride'; document.body.classList.add('ride'); $('#mode').textContent = `${V.fly ? 'fly' : V.K.walker ? 'walk' : 'drive'} · ${V.K.walker ? V.kind.toUpperCase().replace('AT', 'AT-') : V.kind}`; hintFor();
...
Object.assign(W, {
  ...
  exportFrame: o => { const F = W.film, E = W.exporting; if (!F || !E) return null; const n = Math.max(1, Math.ceil(60 / E.fps)), dt = 1 / E.fps / n; for (let k = 0; k < n; k++) simulate(dt); W.last = performance.now();
    if (o && o.skip) { if (F.hold) return { held: true, why: F.hold.why }; E.frames++; return { t: +(window.Sound ? Sound.clock() : F.play.t).toFixed(3), shot: F.play.i, frame: E.frames, done: !F.play.on }; }   /* a stills pass: the film's time goes on exactly as in a take, nothing is drawn */ E.stepping = true; try { W.renderer.render(W.scene, W.camera); } finally { E.stepping = false; } if (F.hold) return { held: true, why: F.hold.why }; const c = E.canvas, ctx = c.getContext('2d'); F.drawFrame(ctx, c.width, c.height); E.frames++; return { jpeg: c.toDataURL('image/jpeg', 0.92).split(',')[1], t: +(window.Sound ? Sound.clock() : F.play.t).toFixed(3), shot: F.play.i, frame: E.frames, done: !F.play.on }; },
