...
function makePlayer(name) {
  ...
  const groups = parts.map(([slot, part, col]) => {
    const g = new THREE.Group(), geom = W.raw.get(part); if (!geom) { loadPrinted(g, part, col); return g; }
...
function loadPrinted(g, part, col) {
  if (!W.props || !W.props.parse) return; g.userData.part = part;
  W.props.parse(`0 FILE p-${part}.ldr\n0 !LDRAW_ORG Unofficial_Model\n1 ${col == null ? 16 : col} 0 0 0 1 0 0 0 1 0 0 0 1 parts/${part}.dat`, `p-${part}.ldr`).then(grp => { if (g.userData.part !== part) return; while (g.children.length) g.remove(g.children[0]); grp.traverse(o => { if (o.isMesh && o.material) for (const m of Array.isArray(o.material) ? o.material : [o.material]) m.fog = true; }); g.add(grp); g.userData.printed = true; }).catch(e => console.warn('printed part', part, e));
}
...
W.filmFace = (rig, name) => { const def = rig && rig.def, part = def && def.faces && def.faces[name]; if (!part || !rig.mounted.head) return false; const g = rig.mounted.head.group; if (g.userData.part === part) return true; rig.mounted.head.part = part; loadPrinted(g, part, def.head); return true; };
...
W.filmCloseup = (rig, size) => {
  ...
  const head = rig.slots.head; head.updateMatrixWorld(true); const c = new THREE.Vector3(0, -12, 0).applyMatrix4(head.matrixWorld), f = new THREE.Vector3(0, 0, -1).applyQuaternion(rig.figure.quaternion).applyAxisAngle(new THREE.Vector3(0, 1, 0), rig.headP.rotation.y);
  ...
  let out = null; try { W.renderReal(W.scene, cam); const src = W.renderer.domElement, cv = document.createElement('canvas'); cv.width = size; cv.height = size; const k = Math.min(src.width, src.height); cv.getContext('2d').drawImage(src, (src.width - k) / 2, (src.height - k) / 2, k, k, 0, 0, size, size); out = cv.toDataURL('image/jpeg', 0.8); } catch (e) { out = null; }
