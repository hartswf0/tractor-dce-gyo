...
const Events = (() => {
  ...
  function paint() { let q = $('#quest'); if (!q) { q = document.createElement('div'); q.id = 'quest'; q.style.cssText = 'position:fixed;left:14px;top:calc(96px + env(safe-area-inset-top));z-index:6;max-width:min(300px,60vw);padding:8px 10px;border-radius:9px;background:rgba(20,24,32,.82);color:#fff;font:11px/1.35 var(--mono,monospace);white-space:pre-line'; q.addEventListener('click', e => { const b = e.target.closest('button[data-skip]'); if (b) { skip(b.dataset.skip); } else if (e.target.closest('button[data-start]')) { start(e.target.closest('button[data-start]').dataset.start); } }); document.body.appendChild(q); }
    const act = [...E.values()].filter(e => e.state === 'active' && !e.key.startsWith('film:')); q.hidden = !act.length && !q.dataset.menu; if (q.hidden) return;
    const esc = t => String(t).replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c])), btn = 'padding:3px 8px;margin:6px 6px 0 0;border:1px solid rgba(255,255,255,.5);border-radius:6px;background:rgba(255,255,255,.12);color:#fff;font:inherit;pointer-events:auto';
    if (act.length) q.innerHTML = act.map(ev => { const p = progress(ev); return '<b>' + esc(ev.def.name) + '</b>\n' + ev.def.objectives.map(o => (o.done(p, ev) ? 'done  ' : '      ') + esc(o.text)).join('\n') + (ev.def.skippable ? '\n<button data-skip="' + esc(ev.key) + '" style="' + btn + '">Skip it: the route opens, no mastery</button>' : ''); }).join('\n\n');
    else q.innerHTML = '<b>Events</b>\n' + Object.keys(DEFS).map(k => '<button data-start="' + esc(k) + '" style="' + btn + '"' + (DEFS[k].pre && !DEFS[k].pre() ? ' disabled' : '') + '>' + esc(DEFS[k].name) + '</button>').join('') + '\n' + Object.keys(DEFS).map(k => esc(DEFS[k].name) + ': ' + esc(DEFS[k].text)).join('\n'); }
  /** `?event=<key>` starts an event once the place is ready and the figure stands; `?events` shows the menu of events on the quest card. */
  function boot() { let Q = null; try { Q = new URLSearchParams(location.search); } catch (e) { return; } const want = Q.get('event'), menu = Q.get('events') != null; if (!want && !menu) return;
    const t = setInterval(() => { if (!W.ready || !W.rig) return; clearInterval(t); if (menu) { paint(); $('#quest').dataset.menu = '1'; paint(); } if (want) setTimeout(() => { const r = start(want); if (!r.ok) console.warn('[kernel] event ' + want + ': ' + r.reason); }, 1500); }, 300); }
  boot();
