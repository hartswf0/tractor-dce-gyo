...
function paintBindings() {
  ...
  b.textContent = 'THERE: ' + (state.there ? 'set' : '—'); b.classList.toggle('go', !!state.there);
}
...
function capture(hit) {
  ...
  if (hit.kind === 'ground' || (state.that && hit.id !== state.that.id)) {
    const p=hit.point.clone();if(hit.box)p.y=hit.box.max.y;state.there = { kind: hit.kind, point: p }; sayLine('THERE is bound. Say the operation.', '');
  } else {
...
function pinDestination() { if(W.build)W.build.wordAnchor=state.there?.point.clone()||null; if(!state.there||!W.build||!W.camera)return;const dir=state.there.point.clone().sub(W.camera.position).normalize();W.build.on=true;W.build.pin={origin:W.camera.position.clone(),dir};W.build.aimRay(W.build.pin.origin,dir); }
function movePiece(hit, dst, copy) {
  const B = W.build, old = B && B.pieces.get(hit.id); if (!old) return false;
  const e = B.ext(old.part, old.rot);
  const t = B.resolvePlacement({p:dst.clone().add(new THREE.Vector3((e[0]+e[1])/2,0,(e[2]+e[3])/2)),kind:'depth',face:'top'}, {part:old.part,rot:old.rot,ignore:copy?null:old.id});
  if (!t || t.blocked) return false;
  if (copy) { const rows=B.addRows([[null,old.part,old.col,t.x,t.y,t.z,old.rot]],false,true); if(rows.length)B.history.push(rows[0].id); return !!rows.length; }
  const before={id:old.id,part:old.part,col:old.col,x:old.x,y:old.y,z:old.z,rot:old.rot,op:old.op};
  B.take(old.id,true);const p=B.add({...before,x:t.x,y:t.y,z:t.z});
  if(!p){B.add(before);return false;}
  B.history.push({before}); if(B.onEdit)B.onEdit({up:[B.toRow(p)]}); return true;
}
function moveProp(hit, dst, copy) {
  const P = W.props, it = hit.item; if (!P || !it || !P.items.has(it.id)) return false; const g = {x:dst.x,y:Math.max(dst.y,W.G.h(dst.x,dst.z)),z:dst.z};
...
async function infer(words, capturedPacket) {
  ...
  if(!window.Ai||!Ai.key()){
    if(local.verb==='build'||local.verb==='unknown'){sayLine('Add an OpenAI key for new creations, or choose a brick in the existing palette.','');W.wbOpen&&W.wbOpen(true);return;}
  ...
  try {
    ...
    if(a.act==='answer'){sayLine(a.say||'No answer returned.','speak');return;}
...
function resetBindings() { if(W.build)W.build.wordAnchor=null; state.that = state.there = state.pending = null; paintBindings();showSelection(null); }
function execute(cmd) {
  ...
  if (cmd.thereWord && !state.there && state.hover && state.hover.id !== state.that?.id) { const point=state.hover.point.clone();if(state.hover.box)point.y=state.hover.box.max.y;state.there={kind:state.hover.kind,point}; }
...
async function recordStart(e) {
  ...
  try{
    ...
    sayLine('Recording. Tap Stop to put the transcript in the word bar.','');
...
function finishRecording(){
  ...
  $('#pttAsk').classList.remove('on');$('#pttAsk').textContent='Talk';
  ...
  sayLine(blob.size?'Transcribing into the word bar…':'No audio captured. Try microphone permission again.','');
  if(blob.size)transcribeRecording();
}
async function transcribeRecording(){
  if(state.transcribing||!state.audioBlob)return;
  if(!window.Ai||!Ai.key()){sayLine('Recording is ready. Add an API key, then Send audio.','');W.wbOpen&&W.wbOpen(true,true);$('#wbKeyBtn').click();return;}
  ...
  try{
    ...
    $('#words').value=j.text;micStatus('transcribed');debug('transcription.done',{characters:j.text.length});
    state.transcriptPacket=packet; sayLine('Transcript ready. Edit it, then press Build / Change to send.','');
...
function drawHand(hand, pose) {
  ...
  plot(pose, P, '#55e57b'); for(const h of state.hands||[hand])plot(h,H,h===hand?'#58a7ff':'#ffcd32');
}
...
function updateHand(hand,now) {
  ...
  const cur=$('#pttCursor');const rect=W.renderer.domElement.getBoundingClientRect();cur.style.left=(rect.left+nx*rect.width)+'px';cur.style.top=(rect.top+ny*rect.height)+'px';cur.className='on '+(state.grab?'object':state.hover?state.hover.kind==='ground'?'ground':'object':'');
  ...
  if(now-state.gestureSince>100&&g.closed!==state.pinched){
    ...
    if(g.closed){
      ...
      if(hit&&hit.kind!=='ground' && !state.pending){
        const start=state.recentHit&&state.recentHit.hit.id===hit.id?state.recentHit.pointer:[nx,ny];
        state.depth=null;state.grab={hit,start,palm,destination:null};state.pointer={x:start[0],y:start[1]};debug('grab.begin',{gesture:g.name,kind:hit.kind,id:hit.id});sayLine('Held '+label(hit)+'. Move your hand; open it to place.','');
      }
    }else if(state.grab){
      const grab=state.grab;state.grab=null;debug('grab.release',{moved:!!grab.destination});
      if(grab.destination){if(W.build)W.build.lift=0;state.that=grab.hit;state.there={kind:'ground',point:grab.destination};execute({verb:'move',needsThat:true,needsThere:true});}
    }
  }
  const ray=worldRay(nx,ny), B=W.build;
  if(ray && B && (state.grab || B.on)) {
    const options=state.grab?{ignore:state.grab.hit.id,part:state.grab.hit.item.part,rot:state.grab.hit.item.rot}:{};
    if(Number.isFinite(state.depth))options.distance=state.depth;
    const other=(state.hands||[]).find(h=>h!==hand);
    if(other && handGesture(other).closed) {
      if(state.depthPalm==null){state.depthPalm=other[0].y;state.depthStart=state.depth||state.hover?.distance||state.grab?.hit.distance||160;}
      state.depth=Math.max(20,Math.min(1800,state.depthStart+(state.depthPalm-other[0].y)*800));options.distance=state.depth;
    }else state.depthPalm=null;
    B.pointerAim={origin:ray.origin,dir:ray.direction,options}; B.aimRay(ray.origin,ray.direction,options);
    if(state.grab && state.grab.hit.kind==='piece') {
      const t=B.target;state.grab.destination=t&&!t.blocked?new THREE.Vector3(t.x,t.y,t.z):null;
      if(t&&state.helper){const e=B.ext(options.part,options.rot);state.helper.box.set(new THREE.Vector3(t.x+e[0],t.y,t.z+e[2]),new THREE.Vector3(t.x+e[1],t.y+e[4],t.z+e[3]));state.helper.material.color.set(t.blocked?0xd8382e:0x2fbf3f);}
    }else if(state.grab){
      const hit=state.hover;if(hit){const dst=hit.point.clone();if(hit.box)dst.y=hit.box.max.y;state.grab.destination=dst;}
    }
  }
...
function chooseHand(result) {
  const hands=result.landmarks||[], labels=result.handedness||result.handednesses||[];
  const rows=hands.map((hand,i)=>({hand,id:labels[i]?.[0]?.categoryName})).filter(r=>r.id);
  let row=rows.find(r=>r.id===state.activeHand);
  if(state.grab)return row?.hand||null;
  const closed=row && handGesture(row.hand).closed ? row : rows.find(r=>handGesture(r.hand).closed);
  row=closed||row||rows[0];
  if(row && state.activeHand!==row.id){state.pointer=null;state.recentHit=null;state.pinched=false;state.gestureSince=0;}
  state.activeHand=row?.id||null;return row?.hand||null;
}
function adjustDepth(delta) {
  state.depth=delta==null?null:Math.max(20,Math.min(1800,(state.depth||state.hover?.distance||160)+delta));
  if(!state.on && W.build){W.build.depthDistance=state.depth;W.build.pointerAim=null;return;}
  if(W.build && !W.build.pointerAim && W.camera){const ray=worldRay(.5,.5);if(ray)W.build.pointerAim={origin:ray.origin,dir:ray.direction,options:{}};}
  if(W.build?.pointerAim){W.build.pointerAim.options.distance=state.depth;W.build.aimRay(W.build.pointerAim.origin,W.build.pointerAim.dir,W.build.pointerAim.options);}
  sayLine(state.depth==null?'Snap to surfaces.':'Depth '+Math.round(state.depth/20)+' studs.','');
}
async function visionLoop() {
  ...
  if (v.readyState >= 2 && v.currentTime !== state.lastVideoTime) {
    ...
    try {
      const hr = state.hand.detectForVideo(v, now); state.hands=hr.landmarks||[]; const hand=chooseHand(hr); state.landmarks=hand;
      ...
      if (state.mode === 'walk') poseWalk(pose);
        ...
        } else { state.hover=null;if(state.grab)debug('grab.cancel',{reason:'tracking lost'});state.grab=null;state.pointer=null;state.recentHit=null;state.gestureSince=0;state.activeHand=null;state.depth=null;if(W.build){W.build.pointerAim=null;W.build.target=null;W.build.ghost.visible=false;}if(state.helper&&state.that)state.helper.box.copy(state.that.box); $('#pttCursor').className=''; state.pinched=false; }
      }
...
async function start() {
  ...
  try {
    ...
    const v=$('#pttVideo');v.srcObject=state.stream;await v.play();state.on=true;if(W.build)W.build.pointerTracking=true;document.body.classList.add('ptt-on');$('#ptt').classList.add('on');b.classList.add('on');b.setAttribute('aria-pressed','true');b.textContent='Stop pointing';W.wbOpen&&W.wbOpen(true,true);sayLine('Aim at an object, close your fist, move, then open to place. Record or type to create.','');micStatus('tap Record');visionLoop();
...
$('#wbBuild').addEventListener('click',e=>{if(!state.on)return;const t=$('#words').value,cmd=parse(t);e.preventDefault();e.stopImmediatePropagation();heard(t,true,state.transcriptPacket);state.transcriptPacket=null;},true);
$('#words').addEventListener('keydown',e=>{if(!state.on||e.key!=='Enter'||e.shiftKey)return;const cmd=parse(e.target.value);e.preventDefault();e.stopImmediatePropagation();const t=e.target.value;heard(t,true,state.transcriptPacket);state.transcriptPacket=null;},true);
...
function addTestBrick(){
  ...
  const B=W.build;
  if(state.there)B.resolvePlacement({p:state.there.point,kind:'depth',face:'top'});
  const p=B.place();sayLine(p?'Brick placed.':'Aim at a clear surface first.','');return p;
}
// Reuse the existing composer and palette. No second key, prompt, brick or undo control.
const controls=document.createElement('div');controls.id='pttActions';
controls.innerHTML='<div id="pttGesture">Aim with either hand. Close to hold; open to place.</div><details id="pttDiagnostics"><summary>Voice tools</summary><meter id="pttLevel" min="0" max="1" aria-label="Microphone level"></meter><button id="pttSendAudio" disabled>Retry transcription</button><button id="pttBrowserVoice">Browser voice</button><audio id="pttPlayback" controls hidden></audio><button id="pttCopyLog">Copy diagnostics</button><pre id="pttLog"></pre></details>';
$('#ptt').appendChild(controls);
$('#wb').insertBefore($('#ptt'),$('#wbRow'));
$('#wb').insertBefore($('#palette'),$('#ptt'));
$('#wbRow').insertBefore($('#pttAsk'),$('#wbBuild'));
...
$('#pttCopyLog').onclick=async()=>{try{await navigator.clipboard.writeText(JSON.stringify(events,null,2));sayLine('Diagnostics copied.','');}catch(e){sayLine('Clipboard unavailable.','');}};
for(const name of ['pointerdown','pointerup','pointermove','keydown'])controls.addEventListener(name,e=>e.stopPropagation());
function fitBuilderSurface() {
  const active=document.body.classList.contains('ptt-on') || document.body.classList.contains('build');
  const stage=$('#stage'); if(!stage)return;
  const top=active?Math.max($('#top').getBoundingClientRect().bottom,$('#find').getBoundingClientRect().bottom):0;
  const bottom=active&&!document.body.classList.contains('wb-off')?$('#wb').getBoundingClientRect().height:0;
  stage.style.top=top+'px';stage.style.bottom=bottom+'px';
  W.engine?.updateRendererSize?.();
}
let fitFrame=0;
const scheduleFit=()=>{cancelAnimationFrame(fitFrame);fitFrame=requestAnimationFrame(fitBuilderSurface);};
new ResizeObserver(scheduleFit).observe($('#wb'));
new MutationObserver(scheduleFit).observe(document.body,{attributes:true,attributeFilter:['class']});
window.addEventListener('resize',scheduleFit);
scheduleFit();
window.PutThatThere={chooseHand,adjustDepth,handGesture,updateHand,recordStart,recordStop,transcribeRecording,diagnostics:()=>events.slice(),beforeStep,afterPose,poseAngles,addTestBrick,parse,heard,infer,tracePacket,execute,capture,pointIntoWorld,setMode,start,stop,state:()=>({on:state.on,mode:state.mode,that:label(state.that),there:!!state.there,pending:state.pending&&state.pending.verb,walking:state.walking,hand:state.activeHand,holding:!!state.grab,inferring:state.inferring,trace:state.trace.length})};
})();
