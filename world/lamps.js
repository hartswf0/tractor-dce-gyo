...
(function () {
'use strict';
const CAP = 900, EVERY = 28, POOL_R = 6;
function radial() {
...
function create({ scene, M }) {
  ...
  L.lay = (G, roads, foot) => {
    ...
    for (const r of roads || []) {
      const w = r.w || 5; if (!r.pts || r.pts.length < 2 || w < 4 || (foot && foot.has(r.kind)) || r.kind === 'cycleway') continue;
    ...
    const posts = new THREE.InstancedMesh(postG, new THREE.MeshStandardMaterial({ color: 0x2b2e33, roughness: .7, metalness: .2 }), spots.length), heads = new THREE.InstancedMesh(headG, new THREE.MeshStandardMaterial({ color: 0x3a3d44, roughness: .6, emissive: 0xffc070, emissiveIntensity: 0 }), spots.length);
    ...
    const glow = new THREE.Points(gg, new THREE.PointsMaterial({ map: L.tex, size: 3.2 * M, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, color: 0xffc070, opacity: 0, fog: false })); glow.name = 'lampGlow'; glow.frustumCulled = false; group.add(glow);
    ...
    const pools = new THREE.Mesh(pg, new THREE.MeshBasicMaterial({ map: L.tex, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, color: 0xffb860, opacity: 0, fog: false, side: THREE.DoubleSide })); pools.name = 'lampPools'; pools.frustumCulled = false; pools.renderOrder = 2; group.add(pools);
  ...
  L.stats = () => ({ lamps: L.n, night: +L.night.toFixed(2), glow: L.parts ? +L.parts.glow.material.opacity.toFixed(2) : 0, pools: L.parts ? +L.parts.pools.material.opacity.toFixed(2) : 0, visible: !!(L.parts && L.parts.glow.visible) });
