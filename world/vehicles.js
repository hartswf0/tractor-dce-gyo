...
function create({ W, M }) {
  ...
  V.lay = async () => {
    ...
    const list = (preset && preset.vehicles) || []; V.laid = W.world; V.n = 0;
    const base = W.rig.pos; let k = 0;
    for (const v of list) {
      ...
      const it = await props.add({ id: `lm-${W.world}-${++k}`, mpd: pr.mpd, x, y, z, yaw: v.yaw || 0, src: { ...op, landmark: true } }, true);
      if (it) V.n++;
    }
    return V.n;
  };
