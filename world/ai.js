...
function status(stage, detail, state = 'working', started = 0) {
  lockModelUi();
  if (typeof Ai.onStatus === 'function') { try { Ai.onStatus(stage, detail, state, started); } catch (e) { } }   // the host page's own status strip (word-to-world.html)
...
const Ai = {
  ...
  /* Words about a build that already stands (a draft, or one read back from the world): one pass, the whole program comes back changed as asked. */
  async edit(prev, words, opts = {}) {
    ...
    const messages = base.concat([{ role: 'user', content: `Change the build: ${words}\nThe words may describe the whole wanted result or just the change. Answer with the corrected FULL program as JSON only: keep everything that was not asked to change at the same coordinates, and apply the change to the rest.` }]);
    const out = await this.request(messages, { key: opts.key || this.key(), signal: opts.signal, stage: 'SOL CHANGING', detail: words.slice(0, 80) });
    const result = compile(out.program), rr = result && result.report || {};
    status('CHANGE READY', `${rr.pieces || 0} pieces · ${rr.props || 0} props · ${rr.floating || 0} floating`, 'ready');
    return { program: out.program, usage: out.j.usage || null, raw: out.raw, responseId: out.j.id || null, messages: messages.concat([{ role: 'assistant', content: out.raw }]) };
  },
