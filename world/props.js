...
class Props {
  ...
  pushOut(pos, r, hgt, step = 30) {
    for (const it of this.near(pos.x, pos.z, r)) {
      const b = it.box; if (b.max.y <= pos.y + step || b.min.y >= pos.y + hgt) continue;
  ...
  /* ───── merge, save, load ───── */
  applyOps({ up, rm }) { let added = 0, removed = 0; if (rm) for (const id of rm) if (this.remove(id, true)) removed++; if (up) for (const r of up) { const p = this.fromRow(r); if (this.items.has(p.id)) continue; this.add(p, true); added++; } if (added || removed) this.dirty = true; return { added, removed }; }
