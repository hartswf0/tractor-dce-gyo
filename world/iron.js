...
function note(text, hold) { if (hold) H.said = { text, until: performance.now() + hold }; else if (H.said && performance.now() < H.said.until) return; else H.said = null; if (text === H.line) return; H.line = text; const el = $('#ironLine'); if (el) el.textContent = text; }   // a refusal holds the line for a moment over the running state
