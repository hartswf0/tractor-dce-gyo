...
function drive(now) {
  ...
  if (tool && !held) {
    ...
    else if (g === 'up') { cue = W.mode === 'walk' ? 'thumb up: get in' : 'thumb up'; if (ready && tool.latched !== 'up') { tool.latched = 'up'; if (W.mode === 'walk') { const pr = W.prompt ? W.prompt() : null; if (pr && pr.on) { W.promptAction(); log('command', { name: 'in', did: pr.text }); } else { note('Nothing to get into here: walk up to a car.'); log('command', { name: 'in', did: null }); } } } }
    else if (g === 'down') { cue = W.mode === 'walk' ? 'thumb down' : 'thumb down: get out'; if (ready && tool.latched !== 'down') { tool.latched = 'down'; if (W.mode !== 'walk') { W.promptAction(); log('command', { name: 'out', mode: W.mode }); } } }
