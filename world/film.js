...
An act makes the player's figure walk (or the ride drive) to a point during the shot. Keep the film to 2-8 shots, name each like a storyboard, vary frames and bearings, and answer with the JSON only.`;
  ...
  function create({ W, M }) {
    const F = {
      ...
      actors: new Map(), builds: new Map(), donors: new Map(), marks: new Map(), rehearse: { on: false, part: null, want: null }, scene: null, cable: null, hang: null, prone: false, rout: null, strikes: null, pending: [], meshes: [], fired: 0,
    };
    ...
    F.text = () => {
      ...
      for (const b of F.builds.values()) if (!/^mark-/.test(b.name)) out.push(`0 !MENTO BUILD "${b.name}" AT ${fmt(b.ax)} ${fmt(-b.az)} ${JSON.stringify(b.program)}`);
      for (const d of F.donors.values()) out.push(`0 !MENTO DONOR "${d.name}" SET ${d.set} AT ${fmt(d.x / M)} ${fmt(-d.z / M)} HEADING ${fmt(d.heading)} SCALE ${fmt(d.scale)}${d.lift ? ' LIFT ' + fmt(d.lift) : ''}`);
      for (const m of F.marks.values()) out.push(`0 !MENTO MARK "${m.name}" AT ${fmt(m.x / M)} ${fmt(-m.z / M)} COL ${m.col}`);
      F.shots.forEach((s, i) => {
    ...
    /** Every figure back on its mark, its register cleared: the shot can run again. */
    F.resetMarks = () => { const sc = F.scene; if (!sc) return 0; let n = 0;
      for (const a of F.actors.values()) { if (!a.rig) continue; a.rig.pos.set(a.x, groundH(a.x, a.z), a.z); a.rig.heading = headingOf(a.heading || 0); a.rig.figure.rotation.y = a.rig.heading; a.rig.speed = 0; a.rig.gait = 0; a.act = null; a.poseNow = a.pose || 'stand'; a.poseUntil = 0; a.down = false; a.fall = null; if (a.perf && window.Perform) { Perform.reset(a.perf); if (a.pose && Perform.PHRASES[a.pose]) Perform.phrase(a.perf, a.pose, 0, { enter: 0.01 }); } if (a.hidden && W.rig) { W.rig.pos.set(a.x, groundH(a.x, a.z), a.z); W.rig.heading = headingOf(a.heading || 0); W.rig.figure.rotation.y = W.rig.heading; } n++; }
      return n; };
    /** The player takes a part: the actor steps out, the player's figure stands on its mark in its costume; shots on that actor now frame the player, and its acts are the player's to walk. */
    F.takePart = name => { const a = F.actors.get(name); if (!a || !a.rig || a.hidden || !W.rig) return false; if (F.rehearse.part) F.givePart();
      a.hidden = true; a.rig.figure.visible = false; F.rehearse.part = name; const sc = F.scene; if (sc && sc.me === 'off') sc.meOffWas = true;
      if (a.figure && Minifig.DEFS[a.figure] && W.setCharacter && W.character !== a.figure) { F.partWas = W.character; W.setCharacter(a.figure); }
      W.rig.figure.visible = true; W.rig.pos.set(a.rig.pos.x, a.rig.pos.y, a.rig.pos.z); W.rig.heading = a.rig.heading; W.rig.figure.rotation.y = a.rig.heading; if (W.rig.cam) W.rig.cam.set = false; if (F.onChange) F.onChange('part'); return true; };
    /** The part goes back to its actor. */
    F.givePart = () => { const name = F.rehearse.part; if (!name) return false; const a = F.actors.get(name); F.rehearse.part = null; if (a) { a.hidden = false; if (a.rig) a.rig.figure.visible = true; }
      if (F.partWas && W.setCharacter) { W.setCharacter(F.partWas); F.partWas = null; } const sc = F.scene; if (sc && sc.meOffWas && W.rig) { W.rig.figure.visible = false; sc.meOffWas = false; } if (F.onChange) F.onChange('part'); return true; };
    ...
    F.stop = () => { const was = F.play.on; truce(false); if (window.Sound && was) { closeFoley(); Sound.arm(false); } if (F.lampLight || F.spotLight) setLamp(false); F.play = { on: false, i: F.play.i, t: 0, all: false }; F.running = false; F.hold = null; F.actState = null; showTitle(null); if (was && F.onChange) F.onChange('stop'); if (F.mode === 'view' && W.rig) { W.camera.fov = gameFov(); W.camera.updateProjectionMatrix(); W.rig.cam.set = false; } return was; };
    ...
    /** A lamp with AIM is a spot: a narrow cone from AT onto AIM (a face in a doorway), lighting that and little else. */
    function setSpot(o) {
      if (!o) { if (F.spotLight) F.spotLight.visible = false; return; }
      if (!F.spotLight) { F.spotLight = new THREE.SpotLight(0xffffff, 1, 20 * M, 0.2, 0.6, 1.2); F.spotLight.name = 'film-spot'; W.scene.add(F.spotLight); W.scene.add(F.spotLight.target); }
      const L = F.spotLight, sp = F.sceneSpawn || { x: 0, z: 0 }, P = a => new THREE.Vector3(sp.x + a[0] * M, a[1] * M + groundH(sp.x + a[0] * M, sp.z + a[2] * M), sp.z + a[2] * M);
      L.visible = true; L.color.setHex(o.color != null ? o.color : 0xffe0b0); L.intensity = o.intensity != null ? o.intensity : 2; L.distance = (o.distance || 20) * M; L.angle = o.angle || 0.12; L.penumbra = o.penumbra != null ? o.penumbra : 0.7;
      L.position.copy(P(o.at)); L.target.position.copy(P(o.aim)); L.target.updateMatrixWorld(); L.castShadow = false;
    }
    function setLamp(on) { const spot = on && typeof on === 'object' && on.aim; setSpot(spot ? on : null); if (spot) { if (F.lampLight) F.lampLight.visible = false; F.lampOn = false; return; }
      if (on && !F.lampLight) { F.lampLight = new THREE.PointLight(0xdfe8ff, 1.6, 45 * M, 1.4); F.lampLight.name = 'film-lamp'; W.scene.add(F.lampLight); } if (F.lampLight) { F.lampLight.visible = !!on; F.lampOn = !!on; const o = on && typeof on === 'object' ? on : null; F.lampLight.color.setHex(o && o.color != null ? o.color : on === 'warm' || (o && o.warm) ? 0xffb066 : 0xdfe8ff); F.lampLight.intensity = o && o.intensity != null ? o.intensity : on === 'warm' ? 2.2 : 1.6; F.lampLight.distance = o && o.distance != null ? o.distance * M : 45 * M; F.lampLight.castShadow = true; F.lampLight.shadow.mapSize.set(1024, 1024); F.lampLight.shadow.bias = -0.002; F.lampLight.shadow.camera.near = 2; F.lampLight.shadow.camera.far = (o && o.distance != null ? o.distance : 45) * M;
    ...
    /** A donor set (a real LEGO model from the forage) stands at a point of the scene, turned and scaled, its feet on the ground plus a lift (a hull sunk to its deck line). */
    async function layDonor(d) {
      if (!window.Donors || !W.donors) throw new Error('no donor loader'); const sp = F.sceneSpawn || { x: 0, z: 0 }; const x = d.abs ? d.x : sp.x + d.x * M, z = d.abs ? d.z : sp.z + d.z * M;
    ...
    F.setup = async () => {
      ...
      const sp = W.spawn || (W.rig && W.rig.pos) || new THREE.Vector3(); F.sceneSpawn = { x: sp.x, z: sp.z };
      if (sc.set && W.filmSet) { const st = sc.set; if (!st.abs) { st.centre = st.centre ? { x: sp.x + st.centre.x * M, z: sp.z + st.centre.z * M } : { x: sp.x, z: sp.z }; if (st.corridor) st.corridor = st.corridor.map(p => [sp.x / M + p[0], sp.z / M + p[1]]); if (st.keepOut) st.keepOut = st.keepOut.map(b => [sp.x / M + b[0], sp.z / M + b[1], sp.x / M + b[2], sp.z / M + b[3]]); st.abs = true; }
        ...
        W.filmSet(st.kind, { centre: st.centre, r: st.r, seed: st.seed, corridor: st.corridor ? st.corridor.map(p => [p[0], p[1]]) : null, keepOut: st.keepOut || null }); }   // the corridor is in metres, absolute
      ...
      if (!sc.abs) { for (const a of sc.actors) { a.x = sp.x + a.x * M; a.z = sp.z + a.z * M; if (a.r) a.r *= 1; } for (const b of sc.builds) { b.x = sp.x + b.x * M; b.z = sp.z + b.z * M; } for (const s of F.shots) { if (s.keysRel) { for (const k of s.keys) { k.pos.y += groundH(sp.x + k.pos.x, sp.z + k.pos.z); k.pos.x += sp.x; k.pos.z += sp.z; k.tgt.y += groundH(sp.x + k.tgt.x, sp.z + k.tgt.z); k.tgt.x += sp.x; k.tgt.z += sp.z; } s.keysRel = false; } if (s.act && s.act.rel) { s.act.x += sp.x; s.act.z += sp.z; delete s.act.rel; } for (const a of s.acts || []) { if (a.via && a.viaRel) { for (const v of a.via) { v.x = sp.x + v.x * M; v.z = sp.z + v.z * M; } delete a.viaRel; } if (a.to && a.rel) { a.to.x = sp.x + a.to.x * M; a.to.z = sp.z + a.to.z * M; delete a.rel; } if (a.aim && typeof a.aim === 'object' && a.rel !== false) { a.aim.x = sp.x + a.aim.x * M; a.aim.z = sp.z + a.aim.z * M; } if (a.look && typeof a.look === 'object' && a.look.rel) { a.look.x = sp.x + a.look.x * M; a.look.z = sp.z + a.look.z * M; delete a.look.rel; } } for (const ev of s.events || []) if (ev.x != null && ev.rel) { ev.x = sp.x + ev.x * M; ev.z = sp.z + ev.z * M; delete ev.rel; } } sc.abs = true; }
      ...
      if (sc.marks && !sc.marksAbs) { sc.marksAbs = true; const COLS = [4, 14, 1, 2, 25, 26, 5, 15]; let k = 0; for (const [name, m] of Object.entries(sc.marks)) { const x = sp.x + m[0] * M, z = sp.z + m[1] * M, col = m[2] != null ? m[2] : COLS[k++ % COLS.length]; F.marks.set(name, { name, x, z, col, heading: m[3] != null ? headingOf(m[3]) : null }); if (!sc.marksHidden) layBuild({ name: 'mark-' + name, x: x - 0.5 * M, z: z - 0.5 * M, program: { name: 'a mark', ops: [{ op: 'slab', x: 0, z: 0, w: 2, d: 2, y: 0, plates: 1, col }] } }); } }   /* a mark is a coloured 2x2 plate on the floor: where a figure stands, walks to, or looks; a scene with marksHidden keeps the marks as places and lays no plates (a set whose floor is in every shot) */
      const donors = (sc.donors || []).map(d => layDonor(d).catch(e => F.log.push('donor ' + (d.name || d.set) + ': ' + (e.message || e))));
      await Promise.all(sc.actors.map(a => layActor(a).catch(e => F.log.push('actor ' + a.name + ': ' + (e.message || e)))).concat(donors));
      ...
      sc.ready = true; if (F.rehearse.on && F.rehearse.want) { const want = F.rehearse.want === 'scene' ? sc.part : F.rehearse.want; F.rehearse.want = null; if (want) F.takePart(want); }
      if (F.onChange) F.onChange('scene');
    };
    F.teardown = keepSet => {
    ...
    function guide(act, st, pos, heading) {
      ...
      else if (act.lead) { const self = [...F.actors.values()].find(x => x.V && x.V.pos === pos); if (self && self.it && self.it.src) self.it.src.nopush = true; const T = F.subject(act.lead), ah = (act.ahead || 2.2) * M, hT = T.heading || 0, tx = T.x + Math.sin(hT) * ah, tz = T.z + Math.cos(hT) * ah, dx = tx - pos.x, dz = tz - pos.z, d = Math.hypot(dx, dz); const along = dx * Math.sin(heading) + dz * Math.cos(heading), TA = F.actors.get(act.lead), vT = TA && TA.rig ? Math.abs(TA.rig.speed || 0) : 0; want = d > 0.4 * M && along > 0 ? Math.atan2(dx, dz) : hT; if (act.speed == null) speed = along > 0.1 * M ? clamp((vT + 1.5 * along) / ((self && self.V.K && self.V.K.cruise || 3) * M), 0, 1) : 0; }   // lead: kept a step ahead of a figure, the way a pushed cart goes: at the figure's own pace, closing the gap, never running off ahead of it
    ...
    function separate() {
      ...
      for (const H of F.actors.values()) {   /* a horse is a body too: nose to tail a capsule, and a standing figure is pushed out of it */
        if (!H.V || !H.V.K || !H.V.K.horse || H.down) continue; const V = H.V, fx = Math.sin(V.heading), fz = Math.cos(V.heading), half = 1.6 * M, r = 0.95 * M;
        for (const a of L) { const p = a.rig.pos, t = Math.max(-half, Math.min(half, (p.x - V.pos.x) * fx + (p.z - V.pos.z) * fz)), cx = V.pos.x + fx * t, cz = V.pos.z + fz * t, dx = p.x - cx, dz = p.z - cz, d = Math.hypot(dx, dz); if (d >= r) continue; const ux = d > 1e-3 ? dx / d : Math.cos(V.heading), uz = d > 1e-3 ? dz / d : -Math.sin(V.heading); p.x = cx + ux * r; p.z = cz + uz * r; }
      }
    }
    ...
    function dismount(a) { const B = F.actors.get(a.riding); if (W.filmUnseat) W.filmUnseat(a.rig); if (B && B.V) { a.rig.pos.copy(B.V.pos); a.rig.heading = B.V.heading; const side = (B.offN = (B.offN || 0) + 1) % 2 ? 1 : -1, off = (B.V.K && B.V.K.horse ? 1.3 : 0.9) * M * side; a.rig.pos.x += Math.cos(B.V.heading) * off; a.rig.pos.z -= Math.sin(B.V.heading) * off; }   /* off to one side, the next rider the other: not inside the horse */ a.rig.pos.y = groundH(a.rig.pos.x, a.rig.pos.z); a.rig.figure.rotation.y = a.rig.heading; a.riding = null; }
    ...
    function showCaption(text, who) { const el = document.getElementById('caption'); if (!el) return; el.hidden = !text; el.dataset.who = who || ''; if (text) { const sp = el.querySelector('span'); sp.textContent = text; const a = who && F.actors.get(who), nm = a && a.label ? a.label : who; sp.dataset.name = who && who !== 'narrator' ? nm : ''; } }   // who: 'narrator' sets the words apart from a character's; an actor's label is the name the caption shows (a part played in several places by several figures)
    ...
    if (window.Perform) {
      ...
      Perform.hooks.line = { sec: ev => (window.Sound ? Sound.lineSec(Sound.lineKey(ev.who, ev.text), ev.text) : 0.3 + String(ev.text || '').split(/\s+/).length * 0.36), play: (ev, sec) => { const narr = ev.narration || /^narrator$/i.test(ev.who || ''); F.caption = { text: ev.text || '', until: F.play.t + sec + 0.4, who: narr ? 'narrator' : ev.who }; showCaption(F.caption.text, F.caption.who); if (window.Sound) Sound.fire('line', { key: ev.file ? null : Sound.lineKey(ev.who, ev.text), who: ev.who, text: ev.text, sec, file: ev.file || null, from: ev.from || 0, voice: ev.voice || null }); } };
    ...
    F.drawFrame = (ctx, w, h) => { drawBand(W.renderer.domElement, ctx, w, h); { const s0 = F.play.on && F.shots[F.play.i], g = (s0 && s0.grade) || (F.scene && F.scene.grade); if (g && !(s0 && s0.title != null && s0.style !== 'hud' && s0.style !== 'list')) grade(ctx, w, h, g); } { const s1 = F.play.on && F.shots[F.play.i]; if (s1 && s1.close) drawClose(ctx, w, h, s1.close, F.play.t); } const s = F.play.on && F.shots[F.play.i]; if (s && s.title != null) drawTitle(ctx, w, h, s, F.play.t); if (F.caption && F.play.on) drawCaption(ctx, w, h, F.caption.text); };
    F.afterRender = () => {
    ...
    F.subject = on => {
      ...
      { const [an, part] = s.split(':'); const a = F.actors.get(an); if (a) { if (a.hidden) { const m = me(); m.heading = W.rig ? W.rig.heading : 0; m.what = a.name; m.id = a.name; return m; }   /* the part is taken: the player stands in for the actor */
        if (a.rig) { const P = new THREE.Vector3(); a.rig.figure.getWorldPosition(P); const flat = a.poseNow === 'prone' && !a.riding; return { x: P.x, z: P.z, y0: a.riding ? P.y - 0.4 * M : groundH(P.x, P.z), h: flat ? 0.9 * M : a.poseNow === 'crouch' ? 1.9 * M : 2.6 * M, r: flat ? 1.2 * M : 0.55 * M, what: a.name, id: a.name, kind: 'figure', heading: a.rig.figure.rotation.y - (a.rig.headP ? a.rig.headP.rotation.y : 0) };   /* where the face points: the body's turn (the walk's heading plus the register's root turn) and the head's own yaw, so a shot from the front finds the face after a look, not the heading the walk left */ }
      ...
      { const d = F.donors.get(s); if (d) return { x: d.x, z: d.z, y0: d.y0, h: d.h, r: d.r, what: d.name, id: null, kind: 'donor', heading: headingOf(d.heading) }; }
      { const m = F.marks.get(s); if (m) return { x: m.x, z: m.z, y0: groundH(m.x, m.z), h: 0.2 * M, r: 1 * M, what: 'mark ' + m.name, id: null, kind: 'mark', heading: m.heading != null ? m.heading : 0 }; }
    ...
    const FILL = { wide: 0.38, medium: 0.6, waist: 0.64, close: 1, low: 0.7, aerial: 0.36, shoulder: 0 };   // waist: head and hands, the frame a performance is read in
    F.stage = sh => {
      ...
      const tgtY = S.y0 + S.h * (frame === 'wide' ? 0.45 : frame === 'close' ? 0.85 : frame === 'waist' ? 0.7 : frame === 'shoulder' ? 0.9 : 0.5);
    ...
    F.loadProgram = (prog, append) => {
      const shots = []; const MARKS = (prog && prog.marks) || {}; const actsOf = list => (list || []).map(a => { const o = { ...a }; if (typeof o.to === 'string' && MARKS[o.to]) o.to = MARKS[o.to].slice(0, 2); if (o.to && Array.isArray(o.to)) o.to = { x: o.to[0], z: o.to[1], rel: true }, o.rel = true; if (Array.isArray(o.via)) { o.via = o.via.map(v => { const p = typeof v === 'string' ? MARKS[v] : v; return p ? { x: p[0], z: p[1] } : null; }).filter(Boolean); o.viaRel = true; }   /* via: waypoints a walk passes through, in order, before its mark (an aisle, a door) */ if (o.to && o.to.rel) o.rel = true; if (o.aim && Array.isArray(o.aim)) o.aim = { x: o.aim[0], z: o.aim[1] }; if (o.look && Array.isArray(o.look)) o.look = { x: o.look[0], z: o.look[1], rel: true }; return o; });
