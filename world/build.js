...
class Build {
  ...
  constructor({ scene, M, geoms, colours, groundH, buildings, rings }) {
    this.scene = scene; this.M = M; this.colours = colours; this.groundH = groundH; this.buildings = buildings; this.rings = rings;
    this.mat = new THREE.MeshStandardMaterial({ color: 0xffffff, vertexColors: true, roughness: .5, metalness: 0 });
    ...
    for (const [part, name] of PARTS) {
      ...
      const sz = g.size, dims = { sx: Math.max(STUD, Math.round(sz.x / STUD) * STUD), sz: Math.max(STUD, Math.round(sz.z / STUD) * STUD), sy: Math.max(PLATE, Math.floor(sz.y / PLATE) * PLATE) };
      const im = new THREE.InstancedMesh(g.geom, this.mat, CAP); im.frustumCulled = false; im.name = 'build:' + part; im.count = 0;   // count grows with the high-water slot: empty slots cost nothing
      ...
      scene.add(im); this.kinds.set(part, { part, name, geom: g.geom, dims, im, free: Array.from({ length: CAP }, (_, i) => CAP - 1 - i) });
    }
  ...
  /** Footprint half sizes of a part at a rotation (0..3), in LDU. */
  half(part, rot) { const d = this.kinds.get(part).dims; return rot % 2 ? { x: d.sz / 2, z: d.sx / 2, y: d.sy } : { x: d.sx / 2, z: d.sz / 2, y: d.sy }; }
  boxOf(p) { const h = this.half(p.part, p.rot); return new THREE.Box3(new THREE.Vector3(p.x - h.x, p.y, p.z - h.z), new THREE.Vector3(p.x + h.x, p.y + h.y, p.z + h.z)); }
  ...
  take(id, quiet) {
    ...
    Q1.setFromAxisAngle(UP, p.rot * Math.PI / 2); return { part: p.part, matrix: new THREE.Matrix4().compose(V1.set(p.x, p.y, p.z), Q1, V2.set(1, 1, 1)), colour: this.colours(p.col), id, rot: p.rot };
  }
  ...
  settle(f) {
    ...
    while (queue.length) {
      const g = queue.shift();
      const pos = new THREE.Vector3().setFromMatrixPosition(g.matrix), half = this.half(g.part, g.rot || 0);
      const top = pos.y + half.y; B1.min.set(pos.x - half.x, top - 2, pos.z - half.z); B1.max.set(pos.x + half.x, top + 2, pos.z + half.z);
  ...
  aimRay(origin, dir) {
    ...
    const h = this.half(this.part, this.rot), f = this.frame, offX = ((h.x * 2) / STUD) % 2 ? STUD / 2 : 0, offZ = ((h.z * 2) / STUD) % 2 ? STUD / 2 : 0;
    let x = best.p.x, z = best.p.z, y;
    if (best.face === 'top') y = best.kind === 'ground' ? best.p.y : best.box.max.y;
    else if (best.face === 'bottom') y = best.box.min.y - h.y;
    else { y = best.box.min.y; if (best.face === 'x-') x = best.box.min.x - h.x; else if (best.face === 'x+') x = best.box.max.x + h.x; else if (best.face === 'z-') z = best.box.min.z - h.z; else z = best.box.max.z + h.z; }
    x = snap(x - f.ax, STUD, offX) + f.ax; z = snap(z - f.az, STUD, offZ) + f.az; y = snap(y + f.datum, PLATE) - f.datum + this.lift * PLATE;
    if (best.kind === 'ground' && y < best.p.y - 4) y += PLATE;
    const box = new THREE.Box3(new THREE.Vector3(x - h.x + 1, y + 1, z - h.z + 1), new THREE.Vector3(x + h.x - 1, y + h.y - 1, z + h.z - 1));
    let blocked = false;
...
window.Build = { PARTS, COLOURS, STUD, PLATE, Build };
})();
