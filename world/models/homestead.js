/* world/models/homestead.js — The Searchers' set as a real model: a frontier homestead, 48 × 48 studs.
   ...
   hitching rail with a horse, a water trough, a wagon wheel, a fence line, a woodpile, a water barrel.
...
const LOG = 70, DARK = 308, STONE = 72, PLANK = 308, EARTH = 19, TRACK = 28;   // the yard is the desert's own tan; the path worn to dark tan
function program() {
  const ops = [];
  ...
  ops.push(G('porch', 0, 0, [slab(12, 20, 24, 3, PLANK), slab(20, 23, 8, 2, PLANK)]));
  ...
  const posts = []; for (const x of [12, 18, 28, 34]) for (let y = 0; y < 6; y++) posts.push(part('3941', DARK, x, 23, y));
  ...
  ops.push(G('horse', 34, 31, [part('4493c01', 70, 0, 0, 0, { plate: 1, rot: 1 })]));
