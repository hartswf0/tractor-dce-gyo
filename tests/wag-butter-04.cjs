...
for(const c of ButterCalibration.CORNERS){const raw=[.2+c.x*.5,.25+c.y*.5,.7-c.z*.4];for(let i=0;i<25;i++){now+=40;ButterInput.handleHands(tracks(raw,now),now);}ButterInput.capture();}
...
const result={phaseAfterTeach,phase:I.phase,verified:I.results.length,max:Math.max(...I.results.map(r=>r.error))};$('#reachClose').click();return result;});assert(calibration.phaseAfterTeach==='verify'&&calibration.phase==='done'&&calibration.verified===8&&calibration.max<.02);
...
assert.deepEqual(errors,[]);console.log(JSON.stringify({rotation,hybrid,reverseHybrid,calibration,mobile,errors},null,2));await browser.close();})().catch(e=>{console.error(e);process.exit(1)});
