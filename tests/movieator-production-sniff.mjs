...
async function assert(cond,msg){if(!cond)failures.push(msg)}
...
await page.goto('http://127.0.0.1:4173/movieator-production.html',{waitUntil:'networkidle',timeout:90000});
...
await shot('00-initial-homer');

// Invariant 1: a prosthetic head exposes no arbitrary crown port.
...
await shot('01-homer-hair-blocked');

// Invariant 2: standard head + headwear with anti-stud can click.
await page.getByRole('button',{name:'CLOSE',exact:true}).click();
await select('#world','scooby-doo');
await select('#figure','fred');
await page.getByRole('button',{name:'HMU',exact:true}).click();
await tab('HAIR / HAT');
const hq=page.locator('#q');await hq.fill('Shaggy Rogers');
const hair=page.locator('[data-probe="21787.dat"]');
await hair.waitFor({state:'visible',timeout:10000});
await page.waitForFunction(()=>{const b=document.querySelector('[data-probe="21787.dat"]');return b&&!b.disabled&&b.textContent.includes('CLICK')},{timeout:15000});
await hair.click();await page.waitForTimeout(500);
assert(!(await page.locator('#portBadge').innerText()).includes('LOAD ERROR'),'Compatible headwear produced LDraw load error');
await shot('02-fred-shaggy-hair-click');

// Invariant 3: whole axe proves a grip shaft and snaps to the right hand.
await page.getByRole('button',{name:'PROPS',exact:true}).click();
await tab('RIGHT HAND');
const pq=page.locator('#q');await pq.fill('Axe with Pick End and Long Handle');
const axe=page.locator('[data-probe="39802.dat"]');
await axe.waitFor({state:'visible',timeout:10000});
await page.waitForFunction(()=>{const b=document.querySelector('[data-probe="39802.dat"]');return b&&!b.disabled&&b.textContent.includes('CLICK')},{timeout:20000});
await axe.click();await page.waitForTimeout(700);
assert(!(await page.locator('#portBadge').innerText()).includes('LOAD ERROR'),'Mechanically verified axe failed to render');
await shot('03-fred-axe-right-hand');

// Invariant 4: clocking preserves the fitted grip while changing orientation.
await tab('CLOCK');
const clock90=page.getByRole('button',{name:'90°',exact:true}).last();
if(await clock90.isEnabled()){await clock90.click();await page.waitForTimeout(500)}
await shot('04-fred-axe-clock-90');

// Invariant 5: PIECES only shows committed assembly, not search candidates.
await page.getByRole('button',{name:'PIECES',exact:true}).click();await page.waitForTimeout(150);
...
await shot('05-pieces-committed');

const state={failures,rig:await page.locator('#rigReady').innerText(),ldraw:await page.locator('#ldrawReady').innerText()};
