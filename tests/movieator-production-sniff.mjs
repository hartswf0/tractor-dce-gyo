...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#rigReady')?.textContent.includes('5/5'),{timeout:120000});
  ...
  // A prosthetic head exposes no arbitrary crown port.
  ...
  // Standard head + headwear with anti-stud can click.
  ...
  await hair.waitFor({state:'visible',timeout:15000});
  ...
  // Whole axe proves a grip shaft and snaps to right hand.
  ...
  await axe.waitFor({state:'visible',timeout:15000});
  ...
  // Clocking changes orientation without breaking the grip selection.
  await tab('CLOCK');
  ...
  // PIECES only shows committed assembly.
...
const state={failures,rig:await page.locator('#rigReady').innerText().catch(()=>''),ldraw:await page.locator('#ldrawReady').innerText().catch(()=>''),console:consoleLines.slice(-120)};
