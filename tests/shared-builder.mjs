...
try {
 ...
 let html=fs.readFileSync('word-to-world.html','utf8').replace(/<script[\s\S]*?<\/script>/g,'').replace(/<link[^>]*>/g,'');
 await page.setContent(html);
 await page.addStyleTag({content:fs.readFileSync('world/shared-builder.css','utf8')});
 await page.addScriptTag({content:three});
 await page.addScriptTag({content:fs.readFileSync('world/build.js','utf8')});
 await page.evaluate(()=>{
 ...
 await page.addScriptTag({content:fs.readFileSync('world/put-that-there.js','utf8')});
