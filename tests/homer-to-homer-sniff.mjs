...
await page.getByRole('button',{name:'ASSEMBLY'}).click();await page.getByRole('button',{name:'HEAD / MAKEUP'}).click();await page.waitForTimeout(900);await page.screenshot({path:'homer-sniff/poseidon-head-mobile.png',fullPage:true});
await page.getByRole('button',{name:'CAST'}).click();await page.getByRole('button',{name:/Odysseus/}).first().click();await page.waitForTimeout(1000);await page.screenshot({path:'homer-sniff/odysseus-full-mobile.png',fullPage:true});
...
await page.getByRole('button',{name:'CAST'}).click();await page.getByRole('button',{name:/Penelope/}).first().click();await page.getByRole('button',{name:'PERFORMANCE'}).click();await page.getByRole('button',{name:'RECOGNITION'}).click();await page.waitForTimeout(900);await page.screenshot({path:'homer-sniff/penelope-recognition-mobile.png',fullPage:true});
