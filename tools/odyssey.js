...
/* ── the plan: Book XVI+ scenes block in plan space (engine/blocking.mjs): a room of named stations (x 0..1 left to right,
   z 0..1 far to near), an INITIAL occupancy, and MOVES between stations over the take's clock. Read from the scene module's
   text (its constants, EXTRA stations, INITIAL and MOVES) and the megaron plan, so no browser module runs here. ── */
const PLAN_W = 24, PLAN_D = 16.5;   // the hall is 24 m across and 16.5 m deep in this build: plan x, z map onto it
const planXZ = st => [+((st.x - 0.5) * PLAN_W).toFixed(1), +(-PLAN_D + st.z * PLAN_D).toFixed(1)];
function readStations(text) { const out = {}; for (const m of text.matchAll(/(\w+):\s*\{\s*x:\s*(-?[\d.]+),\s*z:\s*(-?[\d.]+)\s*\}/g)) out[m[1]] = { x: +m[2], z: +m[3] }; return out; }
function readPlan(id) {
  const f = path.join(HW, 'scenes', id + '.mjs'); if (!fs.existsSync(f)) return null; const text = fs.readFileSync(f, 'utf8'); if (!/const MOVES\s*=\s*\[/.test(text)) return null;
  const plan = /_plans\/(\w[\w-]*)\.mjs/.exec(text), pf = plan ? path.join(HW, 'scenes', '_plans', plan[1] + '.mjs') : null; const stations = pf && fs.existsSync(pf) ? readStations(fs.readFileSync(pf, 'utf8')) : {};
  const extra = /const EXTRA\s*=\s*\{([\s\S]*?)\n\};/.exec(text); if (extra) Object.assign(stations, readStations(extra[1]));
  const consts = {}; for (const m of text.matchAll(/^const ([A-Z][A-Z0-9_]*)\s*=\s*(-?[\d.]+)\s*;/gm)) consts[m[1]] = +m[2];
  const num = expr => { try { return Function(...Object.keys(consts), 'return (' + expr + ');')(...Object.values(consts)); } catch (e) { return NaN; } };
  const initial = {}; const ib = /const INITIAL\s*=\s*\{([\s\S]*?)\n\};/.exec(text);
  if (ib) { for (const m of ib[1].matchAll(/(\w+):\s*"(\w+)"/g)) initial[m[1]] = m[2]; for (const m of ib[1].matchAll(/\/\/([^\n]*)/g)) for (const pair of m[1].matchAll(/\b([a-z]+) ([a-z_]+)\b/g)) if (stations[pair[2]] && !initial[pair[1]]) initial[pair[1]] = pair[2]; }   // the inherited marks are named in the comment beside the spread
  const moves = []; const mb = /const MOVES\s*=\s*\[([\s\S]*?)\n\];/.exec(text);
  if (mb) for (const m of mb[1].matchAll(/\{\s*who:\s*"(\w+)",\s*from:\s*"(\w+)",\s*to:\s*"(\w+)",\s*t0:\s*([^,]+),\s*t1:\s*([^}]+)\}/g)) { const t0 = num(m[4]), t1 = num(m[5]); if (isFinite(t0) && isFinite(t1) && stations[m[2]] && stations[m[3]]) moves.push({ who: m[1], from: m[2], to: m[3], t0, t1 }); }
  return { id: plan ? plan[1] : null, stations, initial, moves, consts };
}
const MUSIC = book => book >= 20 ? 'odyssey/music/hearthside-resolution.ogg' : book <= 4 ? 'odyssey/music/aegean-resolve.ogg' : 'odyssey/music/loom-assembly-night-sail.ogg';   /* the halfworld's albums (Treblo): Bronze Council for the Telemachy, Homecoming for the return */
function compile(id) {
  ...
  if (plan && plan.moves.length) { const k = plan.consts.D > 0 ? total / plan.consts.D : 1; walks.length = 0; for (const m of plan.moves) { if (!cast.some(c => nameOf(c) === m.who)) continue; const [x, z] = planXZ(plan.stations[m.to]); walks.push({ who: m.who, t: +(m.t0 * k).toFixed(2), t1: +(m.t1 * k).toFixed(2), x, z, d: +((m.t1 - m.t0) * k).toFixed(2), from: m.from, to: m.to }); } }   /* the plan's clock is the scene's (D seconds); the voice take's is total: the moves are scaled onto the voice */   // a plan's moves are the walks, on the take's clock
  ...
  const sceneCast = (scene.cast && !Array.isArray(scene.cast)) ? scene.cast : {}, named = cast.filter(c => CAST_DEFS.has(nameOf(c))), actors = named.map((c, i) => { const cm = (sceneCast[c] && typeof sceneCast[c] === 'object') ? sceneCast[c] : ((mem.cast || {})[c] || {}); const has = typeof cm.x === 'number'; const n = nameOf(c), st = plan && plan.initial[n] && plan.stations[plan.initial[n]]; if (st) { const [x, z] = planXZ(st); return { name: n, figure: n, x, z, heading: 180, station: plan.initial[n] }; } return { name: n, figure: n, x: has ? toX(cm.x) : +((i - (named.length - 1) / 2) * 7).toFixed(1), z: has ? toZ(cm.z == null ? 0.6 : cm.z) : -14, heading: 180 }; });   // on the plan's stations when the scene has a plan; else the memory's marks; facing south, the camera's side
  ...
  const posAt = (name, t) => { const A = figures.find(f => f.name === name); if (!A) return null; let x = A.x, z = A.z; for (const w of walks) { if (w.who !== name || w.t > t) continue; x = w.x; z = w.z; } return { x, z }; };   /* where a figure stands at t: its mark, then each walk it has started by then (a walk under way counts as arrived: the look lands where the walker is going) */
  const bearingTo = (a, b, t) => { const A = posAt(a, t), B = posAt(b, t); if (!A || !B) return 180; return ((180 - Math.atan2(B.x - A.x, B.z - A.z) * 180 / Math.PI) % 360 + 360) % 360; };
  /* a speaker's close-up stands in front of the face as it is when the shot starts (film.js reads the body's turn and the head's yaw), so a look that turned the actor is never shot from behind; the facing table below is kept for the wide shots' sides */
  ...
  for (let i = 0; i + 1 < bounds.length; i++) {
    ...
    const shot = { name: speaker ? `${speaker}: ${L[0].text.slice(0, 40)}` : Wk.length ? `${Wk[0].who} crosses` : i === 0 ? 'the hall' : `the room at ${t0.toFixed(0)} s`, on, frame: speaker ? (L[0].sec > 5 ? 'close' : 'medium') : 'wide', from: speaker && figures.some(a => a.name === speaker) ? 'front' : speaker ? 'se' : 's', lens: speaker ? 50 : 32, sec, lamp: true, shift: id, acts: [], events: i === 0 ? events0.slice() : [] };
    for (const w of (plan ? walks.filter(w => w.t < t1 - 1e-6 && (w.t1 == null ? w.t + w.d : w.t1) > t0 + 1e-6) : Wk)) { const a = { who: w.who, to: [w.x, w.z] }; const at = +(w.t - t0).toFixed(2); if (at > 0.05) a.at = at; const sec = (w.t1 != null ? w.t1 : w.t + w.d) - w.t, far = Math.hypot(w.x - (actors.find(q => q.name === w.who) || {}).x || 0, 0); if (sec > 0 && far / sec > 1.8) a.run = true; shot.acts.push(a); }
    ...
    for (const l of L) { const ev = { what: 'SPEAK', who: l.who, text: l.text, at: +(l.t - t0).toFixed(2), for: +l.sec.toFixed(2) }; if (file) { ev.file = file; ev.from = +l.t.toFixed(2); } if (l.dir) ev.note = l.dir; shot.events.push(ev);
      for (const o of figures) if (o.name !== l.who) { shot.events.push({ what: 'PERFORM', who: o.name, verb: 'LOOK', target: l.who, at: +Math.max(0, l.t - t0 + 0.2).toFixed(2), followHead: 0.12, keepGaze: true }); facing[o.name] = bearingTo(o.name, l.who, l.t); } }
  ...
  const hall = { name: 'megaron', ops: [{ op: 'group', name: 'floor', x: -24, z: -32, size: [48, 34, 1], ops: [{ op: 'slab', x: 0, z: 0, w: 48, d: 34, y: 0, plates: 1, col: 19 }] }, { op: 'group', name: 'far wall', x: -24, z: -33, size: [48, 1, 9], ops: [{ op: 'wall', from: [0, 0], to: [48, 0], y: 0, h: 9, col: 28 }, { op: 'band', x: 0, z: 0, w: 48, d: 1, y: 7, h: 1, col: 4 }] }, { op: 'group', name: 'great doors', x: -4, z: -33, size: [8, 1, 7], ops: [{ op: 'cut', x: 0, z: 0, w: 8, d: 1, y: 0, h: 7 }, { op: 'door', x: 0, z: 0, y: 0, facing: 's', col: 70 }, { op: 'door', x: 4, z: 0, y: 0, facing: 's', col: 70 }] }, { op: 'group', name: 'left pillar', x: -15, z: -20, size: [2, 2, 9], ops: [{ op: 'column', x: 0, z: 0, y: 0, h: 9, col: 15 }, { op: 'column', x: 1, z: 0, y: 0, h: 9, col: 15 }, { op: 'column', x: 0, z: 1, y: 0, h: 9, col: 15 }, { op: 'column', x: 1, z: 1, y: 0, h: 9, col: 15 }] }, { op: 'group', name: 'right pillar', x: 13, z: -20, size: [2, 2, 9], ops: [{ op: 'column', x: 0, z: 0, y: 0, h: 9, col: 15 }, { op: 'column', x: 1, z: 0, y: 0, h: 9, col: 15 }, { op: 'column', x: 0, z: 1, y: 0, h: 9, col: 15 }, { op: 'column', x: 1, z: 1, y: 0, h: 9, col: 15 }] }, { op: 'group', name: 'the fire', x: -2, z: -17, size: [4, 4, 2], ops: [{ op: 'box', x: 0, z: 0, w: 4, d: 4, y: 0, h: 1, col: 72 }, { op: 'part', part: '3062b', col: 25, x: 1, y: 1, z: 1 }, { op: 'part', part: '3062b', col: 46, x: 2, y: 1, z: 2 }, { op: 'part', part: '4589', col: 25, x: 1, y: 2, z: 2 }] }] };
  ...
  if (shots.length) { shots[0].score = 'file:' + MUSIC(book); shots[0].fade = 4; }
