...
function compile(id) {
  ...
  return { key: 'odyssey-' + id.toLowerCase(), program: { name: `Odyssey ${id}: ${title}`, time: 'dusk', weather: 'clear', ground: 'flat', story, actors, builds: [{ name: 'megaron', x: 0, z: 0, program: hall }], shots }, report: { id, title, lines: lines.length, poses: poses.length, walks: walks.length, shots: shots.length, total: +(vm ? vm.total : total).toFixed(1), voice: !!file, beat } };
}
