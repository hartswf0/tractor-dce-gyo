...
const TERMS='Hair|Hat|Cap|Helmet|Hood|Cowl|Wig|Crown|Tiara|Headband|Bandana|Bandanna|Mask|Headgear|Headwear|Sombrero|Fedora|Bowler|Beanie|Beret|Turban|Mohawk|Quiff|Ponytail|Pigtail|Braid';
function family(r){const s=`${r.description} ${r.category||''}`.toLowerCase();if(/\bhair\b|\bwig\b|ponytail|pigtail|braid|mohawk|quiff/.test(s))return'hair';if(/helmet/.test(s))return'helmet';if(/\bhood\b|cowl|balaclava/.test(s))return'hood';if(/crown|tiara|diadem/.test(s))return'crown-tiara';if(/headband|bandana|bandanna/.test(s))return'headband-bandana';if(/\bcap\b/.test(s))return'cap';if(/\bhat\b|sombrero|fedora|bowler|beanie|beret|stetson|cowboy|turban/.test(s))return'hat';if(/mask/.test(s))return'mask';return'other-headwear'}
function isHeadwear(r){if(/^Minifig Headwear\b/i.test(r.category||''))return true;if(!/^Minifig\b/i.test(r.description))return false;return new RegExp(`\\b(?:${TERMS})\\b`,'i').test(r.description)}
