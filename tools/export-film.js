  ...
  if (stills) {   /* prove the critical frames before a take: the film runs on its own clock, only the named seconds are drawn, graded and saved */
    ...
    while (k < stills.length) {
      ...
      frames++; if (near && r.jpeg) { const f = path.join(out, `${film}-${tag}-${stills[k].toFixed(1)}.jpg`); fs.writeFileSync(f, Buffer.from(r.jpeg, 'base64')); say('still', stills[k], 'shot', r.shot + 1, f);
        if (args.includes('--dump')) { const d = await page.evaluate(() => { const W = window.__world, M = W.M || 40, sp = (W.film && W.film.sceneSpawn) || { x: 0, z: 0 }, rel = v => v ? [+((v[0] - sp.x) / M).toFixed(1), +(v[1] / M).toFixed(1), +((v[2] - sp.z) / M).toFixed(1)] : null, c = W.camera.position.toArray(); return { cam: rel(c), actors: W.filmActors().map(a => { const A = W.film.actors.get(a.name), h = A && A.rig ? A.rig.figure.rotation.y : A && A.V ? A.V.heading : null; return a.name + ' ' + JSON.stringify(rel(a.figPos || a.pos)) + (h != null ? ' h ' + Math.round(h * 180 / Math.PI) : '') + (a.riding ? ' on ' + a.riding : ''); }) }; }); say('  camera', JSON.stringify(d.cam)); for (const a of d.actors) say('   ', a); }   /* --dump: where the camera and the cast stand at the still, in scene metres */
        k++; }
      if (r.done) break;
    }
