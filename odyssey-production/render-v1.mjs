...
const variant = new URLSearchParams(location.search).get('variant') || 'v1';
...
const hallOps=[
 ...
 {op:'part',part:'85959',col:25,x:0,z:-16,y:1},
 {op:'part',part:'85959',col:46,x:-1,z:-17,y:1},
];
...
const bed=await build('rooted-bed',[
 {op:'slab',x:45,z:-20,w:12,d:12,y:0,plates:1,col:28},
 {op:'box',x:47,z:-18,w:8,d:8,y:0,h:2,col:70},
 {op:'slab',x:47,z:-18,w:8,d:8,y:2,plates:2,col:19},
 {op:'column',x:47,z:-18,y:0,h:5,col:70},
 {op:'box',x:45,z:-19,w:5,d:1,y:0,h:1,col:70},
 {op:'box',x:46,z:-20,w:1,d:5,y:0,h:1,col:70},
]);
const actors={};
...
function mark(name){const p=world(C.stations[name]);if(repaired&&name==='embrace_r')p.x=world(C.stations.embrace_l).x+50;return p;}
...
function cameraAt(t,ts){
 ...
 else if(t<11.86){pos=[390,130,-80];target=[250,62,-369];fov=39;label='THE ORDER';}
 else if(t<18){pos=[55,120,90];target=[-175,55,-233];fov=36;label='HE KNOWS THE BED CANNOT MOVE';}
 else if(t<22.9){pos=[1100,215,-160];target=[1000,50,-310];fov=39;label='THE SECRET · chamber visualization';}
 ...
 else if(t<42.64){pos=[-70,130,75];target=[repaired?-129:-96,60,-172];fov=37;label='AFTER TWENTY YEARS';}
...
function poseAt(who,ts,t,moving){
 ...
 if(ts>=50&&(who==='odysseus'||who==='penelope')){channels['arm.L.pitch']=-1.2;channels['arm.R.pitch']=-1.25;channels['arm.L.out']=.15;channels['arm.R.out']=.15;channels['torso.lean']=.06;}
...
function draw(t){
 ...
 out.font='13px monospace';out.fillStyle='#ddd';out.fillText(`${variant.toUpperCase()} · NATIVE LDRAW · RECORDING ${t.toFixed(2)}s / SOURCE ${ts.toFixed(2)}s`,26,703);
...
function tick(){if(!playing)return;let t=voice.currentTime;if(voice.ended){tailStart??=performance.now();t=C.recordingDuration+(performance.now()-tailStart)/1000;}draw(Math.min(t,55));document.querySelector('#seek').value=t.toFixed(2);if(t>=55){playing=false;return;}raf=requestAnimationFrame(tick);}
document.querySelector('#play').onclick=async()=>{if(exporting)return;playing=true;tailStart=null;if(voice.ended)voice.currentTime=0;await voice.play();tick();};
document.querySelector('#stop').onclick=()=>{playing=false;voice.pause();cancelAnimationFrame(raf);};
document.querySelector('#jump').onclick=()=>{playing=false;voice.pause();const t=+document.querySelector('#seek').value;voice.currentTime=Math.min(t,C.recordingDuration);draw(t);};
