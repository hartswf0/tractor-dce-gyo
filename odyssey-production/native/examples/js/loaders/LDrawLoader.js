...
`
	...
	class LDrawLoader extends THREE.Loader {
		...
		addMaterial( material ) {
			...
			const currentScope = this.getCurrentParseScope();
			...
			matLib[ code ] = material;

			if ( this.parseScopesStack.length > 0 ) {

				const rootScope = this.parseScopesStack[ 0 ];

				if ( rootScope && rootScope !== currentScope ) {

					rootScope.lib[ code ] = material;
 ...
 if (!parseScope.triangles) throw new Error("Missing LDRAW_ORG: "+parseScope.url+" name "+parseScope.fileName+" current "+parentParseScope.currentFileName);
