...
An act makes the player's figure walk (or the ride drive) to a point during the shot. Keep the film to 2-8 shots, name each like a storyboard, vary frames and bearings, and answer with the JSON only.`;
  ...
  function create({ W, M }) {
    ...
    F.view = i => { F.select(i); F.stop(); const s=F.shots[F.sel]; if(s){if(s.plan){const st=F.stage(s.plan);s.keys=st.keys;s.curve=null;s.readout=(s.direction?s.direction+' · ':'')+st.readout;}F.setMode('shot');} return F.sel; };
    F.here = asKey => {
    ...
    function settle(s) {
      if (s.plan) { try { s.bearKeep = null; const st = F.stage(s.plan); s.keys = st.keys; s.curve = null; s.name = s.name || st.name; s.readout = (s.direction?s.direction+' · ':'')+st.readout; if (s.follow) s.bearKeep = st.bearKeep; } catch (e) { F.log.push('stage: ' + (e.message || e)); } }
    ...
    function layBuild(b) {
      ...
      if (W.props && W.props.place) for (const pr of res.props || []) if (pr.kind === 'mpd' && pr.mpd) { const pl = Dsl.propPlace(pr, { ax: b.x, ay, az: b.z }); W.props.place(pr.mpd, pl.x, pl.y, pl.z, pl.yaw, true, { film: true, groundBase: !!b.groundBase, name: 'film-' + b.name }); }   /* a part the brick library does not carry (a helmet, a spear, a sail, a horse) comes through the loader as a prop */
      ...
      // MPD-only assemblies have no brick rows: include their declared envelope for framing.
      for (const pr of res.props || []) { const pl = Dsl.propPlace(pr, { ax: b.x, ay, az: b.z }); lo[0] = Math.min(lo[0], pl.x - pl.w / 2); hi[0] = Math.max(hi[0], pl.x + pl.w / 2); lo[1] = Math.min(lo[1], pl.z - pl.d / 2); hi[1] = Math.max(hi[1], pl.z + pl.d / 2); top = Math.max(top, pl.y + pl.h); }
      if (!Number.isFinite(lo[0])) { lo = [b.x, b.z]; hi = [b.x, b.z]; }
    ...
    F.stage = sh => {
      ...
      if (move === 'push') keys.push(clear(d * (frame === 'close' ? 0.85 : 0.65), bearUsed)); else if (move === 'pull') keys.push(clear(d * 2, bearUsed));
    ...
    F.loadProgram = (prog, append) => {
      ...
      for (const sh of (prog && prog.shots) || []) {
        ...
        const shot = { direction: sh.direction, name: sh.name || `${plan.frame} on ${plan.on}`, keys: [{ pos: new THREE.Vector3(0, 4 * M, 0), tgt: new THREE.Vector3(0, 2 * M, -10 * M), fov: 50 }], sec: clamp(+sh.sec || F.sec, 0.5, 120), act, set, plan, follow: !!sh.follow, acts: actsOf(sh.acts), events: eventsOf(sh.events) };
        ...
        if (sh.cameraEnd && shot.keysRel) { const e = sh.cameraEnd; shot.keys.push({ pos: new THREE.Vector3(...e.pos).multiplyScalar(M), tgt: new THREE.Vector3(...e.tgt).multiplyScalar(M), fov: clamp(+sh.lens || 45, 5, 120) }); }
