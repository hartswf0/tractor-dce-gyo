...
const KINDS = {
  horse: { cruise: 5, boost: 10, turn: 2.1, fly: false, hover: 0, ram: 0, accel: 4, grip: 12 },
...
function create({ prop, M, groundH, aabbs }) {
  ...
  if(kind === "boat" && prop.src?.water) V.groundH=()=>prop.src.water.y;
  V.pos.y = V.groundH(V.pos.x, V.pos.z) + K.hover * M;
  return V;
}
function waterContains(w,x,z,r=0){ return x>=w.minX+r && x<=w.maxX-r && z>=w.minZ+r && z<=w.maxZ-r; }
...
function step(V, dt, ctx) {
  ...
  const previous=V.pos.clone();
  if (V.fly) stepFly(V, dt, ctx); else stepDrive(V, dt, ctx);
  const water=V.prop.src && V.prop.src.water;
  if (V.kind === "boat" && water) {
    if (!waterContains(water,V.pos.x,V.pos.z,V.r)) { V.pos.copy(previous); V.speed=0; V.vel.set(0,0,0); }
    V.pos.y=water.y; V.pitch=0; V.roll=Math.sin(V.t*1.5)*0.015; V.nose=0;
  }
  if(V.kind==='horse'&&window.HorseMotion)HorseMotion.step(V,dt);
  place(V);
  return !V.done;
}
function stepDrive(V, dt, ctx) {
  ...
  const yawRate = V.kind === "horse" ? -i.x*K.turn*(sp>0.1*M ? 1 : .55) : clamp(yawWant, -yawMax, yawMax); V.heading += yawRate * dt;
...
function park(V) { V.speed = 0; if (V.fly) { V.pitch = 0; V.roll = 0; } V.landing = 0; V.airborne = false; V.bob = 0; V.nose = 0; V.lean = 0; V.slip = 0; V.steer = 0; if (V.wheelSet) for (const w of V.wheelSet) w.rotation.y = 0; if (V.legs) for (const g of V.legs) g.rotation.x = 0; place(V); V.prop.x = V.pos.x; V.prop.y = V.pos.y - V.K.hover * V.M; V.prop.z = V.pos.z; V.prop.yaw = V.heading / (Math.PI / 2); V.group.updateMatrixWorld(true); V.prop.box = V.prop.localBox ? V.prop.localBox.clone().applyMatrix4(V.group.matrixWorld) : new THREE.Box3().setFromObject(V.group); }
...
window.Drive = { waterContains, KINDS, kindOf, create, step, land, park, camera, forward, settle, gait, wheels };
})();
