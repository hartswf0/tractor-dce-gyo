...
class Crowd {
  ...
  spawn(kind, x, z, seed) {
    ...
    if (window.__world?.world === 'odyssey') kind = 'citizen';
    const def = window.__world?.world === 'odyssey' ? {...Minifig.DEFS.sailor, weapon:null} : kind === 'trooper' ? Minifig.DEFS.trooper : kind === 'rebel' ? Minifig.DEFS.rebel : Minifig.citizen(seed), i = this.free.pop();
  ...
  step(dt, ctx) {
    ...
    for (const n of this.npcs.slice()) {
      ...
      if (n.kind === 'trooper') {
        const sees = ctx.player.alive && !ctx.player.peace && dP < 40 && (dP < 12 || ctx.los(n.pos, ctx.player.pos));   // peace: nobody shoots a builder at work
        if (sees) { n.state = 'engage'; goalHeading = Math.atan2(toP.x, toP.z); aim = 1; if (dP < 6) { want = -1.2; } else if (dP > 24) want = 1.6; if (n.cd <= 0) { n.cd = 1.5 + Math.random(); this.poseTemplate(n); const mz = Minifig.muzzleAt(this.tmpl.slots.weaponR.matrixWorld, V2); V3.copy(ctx.player.pos); V3.y += 1.3 * M; V3.sub(mz).normalize(); V3.x += (Math.random() - .5) * 0.12; V3.z += (Math.random() - .5) * 0.12; ctx.bolts.fire(mz, V3, 'npc'); } }
        else if (ctx.tie.flying && Math.hypot(ctx.tie.pos.x - n.pos.x, ctx.tie.pos.z - n.pos.z) < 70 * M) { n.state = 'engage'; V2.subVectors(ctx.tie.pos, n.pos); goalHeading = Math.atan2(V2.x, V2.z); aim = 1; if (n.cd <= 0) { n.cd = 2 + Math.random(); this.poseTemplate(n); const mz = Minifig.muzzleAt(this.tmpl.slots.weaponR.matrixWorld, V3); V2.sub(mz).normalize(); ctx.bolts.fire(mz, V2, 'npc'); } }
  ...
  patrol(n, dt, speed) {
    ...
    let pts = n.road.pts; const M = this.M; if (!pts || !pts.length) { n.road = null; return null; } n.seg = clamp(n.seg || 0, 0, pts.length - 1); let tgt = pts[n.seg];
    if (Math.hypot(tgt.x * M - n.pos.x, tgt.z * M - n.pos.z) < 1.5 * M) { n.seg += n.dir; if (n.seg < 0 || n.seg >= pts.length) { n.dir = -n.dir; n.seg += 2 * n.dir; n.seg = clamp(n.seg, 0, pts.length - 1); if (Math.random() < 0.3) { this.pickRoad(n); } } pts = n.road?.pts || []; tgt = pts[n.seg]; if (!tgt) { n.road = null; return null; } }
