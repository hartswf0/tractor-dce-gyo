...
class Props {
  ...
  async add(p, quiet) {
    ...
    if (p.src && p.src.groundBase && !it.box.isEmpty()) {
      // Native DAT origins are attachment points, often at the top of rock panels.
      // Seat the measured lowest vertex at the requested ground height.
      const lift = p.y - it.box.min.y; wrap.position.y += lift;
      yawG.updateMatrixWorld(true); it.box.setFromObject(yawG);
    }
  ...
  near(x, z, r) { const out = []; for (const it of this.items.values()) { if (!it.box || (it.group && !it.group.visible)) continue; const b = it.box; if (x + r < b.min.x || x - r > b.max.x || z + r < b.min.z || z - r > b.max.z) continue; out.push(it); } return out; }
  ...
  blast(pt, r, vel) { if(window.__world?.rehearsalProtectSet)return 0;   // a kit (a real set) stays whole
