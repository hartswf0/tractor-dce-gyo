...
function roads(G, list, M) {
  const ancient = window.__world?.world === 'odyssey';
  const S = stripper(G, M), c = lin(ancient ? 0x94774f : 0x3d4148), cl = lin(0xd8d4c4), edge = lin(0xe8e4d8);
  const wallC = lin(0x6e7073), pierC = lin(0x55585e); G.decks = [];
  for (const r of list) {
    const pts = r.pts, w = r.w || 5; if (!wheels(r)) continue;
    const lift = LIFT[r.kind] || 0.23, marks = !ancient && w >= 5 && r.kind !== 'service';
    ...
    S.along(pts, (a, b, s) => { S.quad(a, b, w, c, lift); if (marks && s % 2 === 0) S.quad(a, b, 0.3, cl, lift + 0.02); if (!ancient && w >= 9) { S.quad(a, b, 0.15, edge, lift + 0.02, w / 2 - 0.35); S.quad(a, b, 0.15, edge, lift + 0.02, -(w / 2 - 0.35)); if (s % 2 === 1) { S.quad(a, b, 0.15, edge, lift + 0.02, w / 4); S.quad(a, b, 0.15, edge, lift + 0.02, -w / 4); } } });
  }
...
function streets(G, win, M) {
  if (window.__world?.world === 'odyssey') {
    const S = stripper(G,M), L = layers(G,M);
    for (const r of win.roads || []) { if (!r.pts || r.pts.length < 2 || r.kind === 'river') continue;
      const width = r.w || 3, lift = LIFT[r.kind] || .23;
      S.along(r.pts,(a,b)=>{ if(!wheels(r)) S.quad(a,b,width,lin(0x94774f),lift); L.strip(a,b,width,lift); }); }
    for(const a of win.areas || []) { if(a.kind==='water' && a.ring?.length>2) S.polygon(a.ring,lin(COVER.water[0]),COVER.water[1]); }
