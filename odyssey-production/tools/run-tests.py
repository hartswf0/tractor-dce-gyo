...
for id,file,scope in suite:
 p=subprocess.run(['node','tools/'+file],capture_output=True,text=True)
...
Path('test-results.json').write_text(json.dumps(dict(runAt=datetime.datetime.now(datetime.timezone.utc).isoformat(),tests=results),indent=2))
