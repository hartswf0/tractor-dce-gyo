...
export async function nativeCore(canvas){
 ...
 const loader=new THREE.LDrawLoader(manager);loader.path='./native/ldraw/';loader.separateObjects=true;loader.smoothNormals=false;loader.setFileMap(await(await fetch('ldraw-map.json')).json());
 ...
 async function raw(id,col=70){const k=id+'-'+col;if(!cache.has(k)){cache.set(k,props.parse(`0 FILE ${k}.ldr\n0 !LDRAW_ORG Unofficial_Model\n1 ${col} 0 0 0 1 0 0 0 1 0 0 0 1 parts/${id}.dat`,k+'.ldr').then(finish));}const base=await cache.get(k);await saveJSON('native-progress',{part:id,stage:'parsed'});const clone=base.clone(true);return clone;}
...
export async function saveJSON(name,data){await fetch('/evidence/'+name+'.json',{method:'POST',body:JSON.stringify(data,null,2)});}
