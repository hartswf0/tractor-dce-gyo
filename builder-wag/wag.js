...
const cursors=new Map();for(const side of ['Left','Right']){const n=document.createElement('div');n.className='wagCursor '+side.toLowerCase();n.textContent=side[0];n.hidden=true;$('#stage').append(n);cursors.set(side,n);}
function paint(){const s=C.state;$('#wagStatus').textContent=s.message;place.disabled=s.panel||!W.ready||(mode==='move'&&(!s.preview.size||[...s.preview.values()].some(t=>t.blocked)));precision.hidden=!s.selected||mode==='place';const p=W.build?.pieces.get(s.selected),d=s.preview.get(s.selected)||p;coords.textContent=d?`X ${Math.round(d.x)} · Y ${Math.round(d.y)} · Z ${Math.round(d.z)}`:'';for(const b of precision.querySelectorAll('button'))b.disabled=s.panel;
 for(const [side,n]of cursors){const h=s.hands.get(side);n.hidden=s.panel||!h?.visible||!h.pointer||!!h.grab;if(!n.hidden){n.style.left=(h.pointer.x*100)+'%';n.style.top=(h.pointer.y*100)+'%';n.classList.toggle('target',!!h.hover||!!h.grab);n.textContent=side[0]+(h.grab?' •':'');}}
