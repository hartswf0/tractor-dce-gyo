...
(function(){
 ...
 function start(){if(S.tx)return note('Release the piece before calibration.','HELD');I.phase='teach';I.index=0;I.samples=[];I.results=[];I.window=[];I.profileBefore=I.profile;I.profile=null;I.primary=I.tool=null;I.signature=signature();I.verifyAt=0;I.captureAt=0;I.doneAt=0;I.armed=true;I.palmReference=null;$('#reachPanel').hidden=false;closeDrawers();paint();}
 ...
 function capture(){
  ...
  const raw=[0,1,2].map(k=>C.median(I.window.map(p=>p[k])));I.samples.push({raw,target:C.CORNERS[I.index]});I.lastCaptured=raw;I.window=[];I.captureAt=0;I.armed=false;
 ...
 function paint(){const c=C.CORNERS[I.index]||C.CORNERS[0];$('#reachTitle').textContent=I.phase==='teach'?'TEACH REACH · '+(I.index+1)+' / 8':I.phase==='verify'?'CHECK REACH · '+(I.index+1)+' / 8':I.phase==='done'?'REACH CHECK COMPLETE':'REPEAT CALIBRATION';
  if(['teach','verify'].includes(I.phase))$('#reachText').textContent=(I.phase==='teach'?'Put the grip input comfortably ':'Reach the highlighted corner: ')+(c.x?'right':'left')+', '+(c.y?'low':'high')+'. Tool input '+(c.z?'up / away':'down / near')+'. '+(I.phase==='teach'?'Hold steady: capture and the next corner are automatic.':'Hold the cursor inside the target for 0.6 seconds.');
  $('#reachCapture').hidden=true;$('#reachRetry').hidden=!['failed','done'].includes(I.phase);$('#reachMode').disabled=['verify'].includes(I.phase);$('#reachCanvas').hidden=!['teach','verify'].includes(I.phase);
 }
 function handleHands(tracks,now){
  ...
  if(['teach','verify','failed','done'].includes(I.phase)){
   if(obs){if(!I.armed&&I.lastCaptured&&I.phase==='teach'){const previous=C.CORNERS[I.index-1],next=C.CORNERS[I.index];I.armed=['x','y','z'].every((k,i)=>next[k]===previous[k]||(obs.raw[i]-I.lastCaptured[i])*(next[k]-previous[k])*(i===2?-1:1)>.06);}
    if(I.phase==='teach'&&I.armed){I.window.push(obs.raw);if(I.window.length>24)I.window.shift();if(C.stable(I.window)){if(!I.captureAt)I.captureAt=now;if(now-I.captureAt>=800){capture();return true;}}else I.captureAt=0;}else{I.window=[];I.captureAt=0;}
    if(I.phase==='verify'){const mapped=C.map(I.profile,obs.raw),error=C.error(mapped,C.CORNERS[I.index]);I.liveError=error;I.mapped=mapped;
     if(error<.08){if(!I.verifyAt){I.verifyAt=now;I.verifyErrors=[];}I.verifyErrors.push(error);if(now-I.verifyAt>=600){I.results.push({corner:I.index,error:Math.max(...I.verifyErrors),holdMs:now-I.verifyAt});I.verifyAt=0;feedback('click',.4);if(++I.index===8){I.phase='done';I.doneAt=now;I.signature=signature();save();$('#reachText').textContent='8 / 8 reached within 8% axis tolerance. Worst held error '+Math.round(Math.max(...I.results.map(r=>r.error))*100)+'% of axis range. This checks control reach, not physical distance.';}paint();}}else I.verifyAt=0;
    }
   }else{I.window=[];I.captureAt=0;I.verifyAt=0;}return true;
  }
 ...
 function frame(now){if(I.phase==='done'&&now-I.doneAt>1800){I.phase='idle';$('#reachPanel').hidden=true;$('#reachCanvas').hidden=true;H.mustOpen=true;note('Reach calibrated · open hands, then pinch to build.','READY');}if(H.active&&!I.offered){I.offered=true;if(!I.profile&&!S.tx)start();}for(const [id,v] of B.visuals){if(!v.tag.hidden&&I.phase!=='idle')v.tag.textContent=id===I.primary?'GRIP · reach the corner':id===I.tool?'TOOL · near / away':v.tag.textContent;}if(I.phase==='teach'){$('#reachCapture').disabled=!C.stable(I.window);$('#reachStatus').textContent=!I.observation?'Show the assigned inputs':!I.armed?'Move to the next corner':I.captureAt?'Capturing '+Math.min(100,Math.round((now-I.captureAt)/8))+'% · keep still':'Hold steady · automatic capture';$('#reachProgress').value=I.captureAt?Math.min(1,(now-I.captureAt)/800):0;if(I.palmCue)$('#reachStatus').textContent+=' · Palm '+(I.palmCue.valid?I.palmCue.relativeDistance.toFixed(2)+'× relative distance cue':'shape changed; size cue ignored');}
  if(I.phase==='verify'){$('#reachProgress').value=I.verifyAt?Math.min(1,(now-I.verifyAt)/600):0;}if(I.phase==='verify')$('#reachStatus').textContent=I.observation?'Error '+Math.round((I.liveError||0)*100)+'% · hold '+Math.min(100,Math.round((I.verifyAt?(now-I.verifyAt):0)/6))+'%':'Input lost · waiting';
 ...
 $('#stage').insertAdjacentHTML('beforeend','<canvas id="reachCanvas" hidden></canvas><section id="reachPanel" hidden aria-label="Reach calibration"><b id="reachTitle"></b><select id="reachMode" aria-label="Input roles"><option value="hands">Two hands</option><option value="mouse-hand">Mouse holds · hand depth</option><option value="hand-mouse">Hand holds · mouse depth</option></select><p id="reachText"></p><output id="reachStatus"></output><progress id="reachProgress" max="1" value="0" aria-label="Steady pose capture progress"></progress><div><button id="reachCapture">Capture</button><button id="reachRetry" hidden>Repeat</button><button id="reachClose">Close</button></div></section>');
